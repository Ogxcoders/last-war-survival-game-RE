	...
	public string GenerateResponseHeader(HTTPRequest request, Credentials credentials, bool isProxy = false)
	{
		try
		{
			...
			case AuthenticationTypes.Digest:
			{
				...
				string text5 = (isProxy ? "CONNECT" : request.MethodType.ToString().ToUpper());
				string text6 = (isProxy ? (request.CurrentUri.Host + ":" + request.CurrentUri.Port) : request.CurrentUri.GetRequestPathAndQueryURL());
				if (text4 == null)
				{
				...
				else if (text4.Contains("auth-int"))
				{
					...
					string text7 = $"{text5}:{text6}:{array.CalculateMD5Hash()}".CalculateMD5Hash();
					empty2 = $"{empty}:{Nonce}:{text2}:{text}:{text4}:{text7}".CalculateMD5Hash();
				}
				else
				{
					...
					text4 = "auth";
					string text8 = (text5 + ":" + text6).CalculateMD5Hash();
					empty2 = $"{empty}:{Nonce}:{text2}:{text}:{text4}:{text8}".CalculateMD5Hash();
				}
				string text9 = $"Digest username=\"{credentials.UserName}\", realm=\"{Realm}\", nonce=\"{Nonce}\", uri=\"{text6}\", cnonce=\"{text}\", response=\"{empty2}\"";
				if (text4 != null)
				{
					text9 = text9 + ", qop=\"" + text4 + "\", nc=" + text2;
				}
				if (!string.IsNullOrEmpty(Opaque))
				{
					text9 = text9 + ", opaque=\"" + Opaque + "\"";
				}
				return text9;
			}
			}
