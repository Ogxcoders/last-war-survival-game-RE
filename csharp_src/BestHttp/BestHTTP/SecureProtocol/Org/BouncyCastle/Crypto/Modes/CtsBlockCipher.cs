...
public class CtsBlockCipher : BufferedBlockCipher
{
	...
	public override int DoFinal(byte[] output, int outOff)
	{
		...
		if (forEncryption)
		{
			...
			((cipher is CbcBlockCipher) ? ((CbcBlockCipher)cipher).GetUnderlyingCipher() : cipher).ProcessBlock(buf, num, output, outOff);
			Array.Copy(array, 0, output, outOff + num, length);
		}
		else
		{
			byte[] array2 = new byte[num];
			((cipher is CbcBlockCipher) ? ((CbcBlockCipher)cipher).GetUnderlyingCipher() : cipher).ProcessBlock(buf, 0, array, 0);
