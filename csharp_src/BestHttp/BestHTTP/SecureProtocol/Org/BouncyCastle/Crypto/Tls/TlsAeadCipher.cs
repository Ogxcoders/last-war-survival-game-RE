...
public class TlsAeadCipher : TlsCipher
{
	...
	public virtual byte[] EncodePlaintext(long seqNo, byte type, byte[] plaintext, int offset, int len)
	{
		...
		case 2:
		{
			...
			{
				array[i] ^= encryptImplicitNonce[i];
			}
			break;
		}
		default:
		...
		byte[] array2 = new byte[record_iv_length + outputSize];
		if (record_iv_length != 0)
		{
			Array.Copy(array, array.Length - record_iv_length, array2, 0, record_iv_length);
		}
		int num = record_iv_length;
	...
	public virtual byte[] DecodeCiphertext(long seqNo, byte type, byte[] ciphertext, int offset, int len)
	{
		...
		case 2:
		{
			...
			{
				array[i] ^= decryptImplicitNonce[i];
			}
			break;
		}
		default:
