...
public class TlsSrpKeyExchange : AbstractTlsKeyExchange
{
	...
	{
		TlsSrpUtilities.WriteSrpParameter(mSrpClient.GenerateClientCredentials(mSrpSalt, mIdentity, mPassword), output);
	...
	public override byte[] GeneratePremasterSecret()
	{
		try
		{
			return BigIntegers.AsUnsignedByteArray((mSrpServer != null) ? mSrpServer.CalculateSecret(mSrpPeerCredentials) : mSrpClient.CalculateSecret(mSrpPeerCredentials));
		}
		catch (CryptoException alertCause)
		{
			throw new TlsFatalAlert(47, alertCause);
