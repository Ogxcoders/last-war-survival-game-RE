	...
	public static ECDomainParameters ReadECParameters(int[] namedCurves, byte[] ecPointFormats, Stream input)
	{
		try
		{
			...
			case 1:
			{
				...
				BigInteger b2 = ReadECFieldElement(bigInteger3.BitLength, input);
				...
				ECCurve curve2 = new FpCurve(bigInteger3, a2, b2, bigInteger4, bigInteger5);
			...
			case 2:
			{
				...
				if (!ECBasisType.IsValid(num2))
				{
					throw new TlsFatalAlert(47);
				}
				int num3 = ReadECExponent(num, input);
				int k = -1;
				int k2 = -1;
				if (num2 == 2)
				{
				...
				BigInteger b = ReadECFieldElement(num, input);
				byte[] encoding = TlsUtilities.ReadOpaque8(input);
				BigInteger bigInteger = ReadECParameter(input);
				BigInteger bigInteger2 = ReadECParameter(input);
				ECCurve curve = ((num2 == 2) ? new F2mCurve(num, num3, k, k2, a, b, bigInteger, bigInteger2) : new F2mCurve(num, num3, a, b, bigInteger, bigInteger2));
