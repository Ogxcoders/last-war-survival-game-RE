...
public class TlsBlockCipher : TlsCipher
{
	...
	public virtual byte[] EncodePlaintext(long seqNo, byte type, byte[] plaintext, int offset, int len)
	{
		...
		if (useExplicitIV)
		{
			...
			Array.Copy(explicitIV, 0, array, num5, blockSize);
			num5 += blockSize;
		}
		int num6 = num5;
		...
		if (!encryptThenMac)
		{
			byte[] array2 = mWriteMac.CalculateMac(seqNo, type, plaintext, offset, len);
		...
		if (encryptThenMac)
		{
			byte[] array3 = mWriteMac.CalculateMac(seqNo, type, array, 0, num5);
	...
	public virtual byte[] DecodeCiphertext(long seqNo, byte type, byte[] ciphertext, int offset, int len)
	{
		...
		if (encryptThenMac)
		{
			...
			if (!Arrays.ConstantTimeAreEqual(mReadMac.CalculateMac(seqNo, type, ciphertext, offset, len - size), b))
			{
		...
		if (!encryptThenMac)
		{
			...
			byte[] a = mReadMac.CalculateMacConstantTime(seqNo, type, ciphertext, offset, num6, num2 - size, randomData);
	...
	protected virtual int CheckPaddingConstantTime(byte[] buf, int off, int len, int blockSize, int macSize)
	{
		...
		int num3 = 0;
		byte b2 = 0;
		if ((TlsUtilities.IsSsl(context) && num2 > blockSize) || macSize + num2 > len)
		{
			num2 = 0;
		}
		else
