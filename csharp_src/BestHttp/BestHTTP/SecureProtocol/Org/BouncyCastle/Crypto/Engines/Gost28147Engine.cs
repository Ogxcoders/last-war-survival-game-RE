...
public class Gost28147Engine : IBlockCipher
{
	...
	private void Gost28147Func(int[] workingKey, byte[] inBytes, int inOff, byte[] outBytes, int outOff)
	{
		...
		else
		{
			...
			{
				int num8 = 7;
				while (num8 >= 0 && (l != 2 || num8 != 0))
				{
					int num9 = num;
					num = num2 ^ Gost28147_mainStep(num, workingKey[num8]);
					num2 = num9;
					num8--;
				}
			}
	...
	{
		return Arrays.Clone(((byte[])sBoxes[Platform.ToUpperInvariant(sBoxName)]) ?? throw new ArgumentException("Unknown S-Box - possible types: \"Default\", \"E-Test\", \"E-A\", \"E-B\", \"E-C\", \"E-D\", \"D-Test\", \"D-A\"."));
	}
	...
	public static string GetSBoxName(byte[] sBox)
	{
		...
		{
			if (Arrays.AreEqual((byte[])sBoxes[key], sBox))
			{
				return key;
			}
		}
