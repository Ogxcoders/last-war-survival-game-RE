...
public class ThreefishEngine : IBlockCipher
{
	...
	internal static ulong BytesToWord(byte[] bytes, int off)
	{
		...
		int num = off;
		return ((ulong)bytes[num++] & 0xFFuL) | (((ulong)bytes[num++] & 0xFFuL) << 8) | (((ulong)bytes[num++] & 0xFFuL) << 16) | (((ulong)bytes[num++] & 0xFFuL) << 24) | (((ulong)bytes[num++] & 0xFFuL) << 32) | (((ulong)bytes[num++] & 0xFFuL) << 40) | (((ulong)bytes[num++] & 0xFFuL) << 48) | (((ulong)bytes[num++] & 0xFFuL) << 56);
	}
