...
public class Dstu7624Engine : IBlockCipher
{
	...
	private void WorkingKeyExpandEven(ulong[] workingKey, ulong[] tempKey)
	{
		...
		while (true)
		{
			...
			{
				internalState[k] ^= array2[k];
			}
			EncryptionRound();
			for (int l = 0; l < wordsInBlock; l++)
			{
				internalState[l] += array2[l];
			}
			...
			if (wordsInKey != wordsInBlock)
			{
				...
				EncryptionRound();
				for (int num3 = 0; num3 < wordsInBlock; num3++)
				{
					internalState[num3] ^= array2[num3];
				}
				EncryptionRound();
				for (int num4 = 0; num4 < wordsInBlock; num4++)
				{
					internalState[num4] += array2[num4];
				}
				Array.Copy(internalState, 0, roundKeys[num], 0, wordsInBlock);
				if (roundsAmount == num)
				{
	...
	private void DecryptBlock_128(byte[] input, int inOff, byte[] output, int outOff)
	{
		...
		while (true)
		{
			...
			byte num8 = T0[num4 & 0xFF];
			byte b = T1[(num4 >> 8) & 0xFF];
			byte b2 = T2[(num4 >> 16) & 0xFF];
			byte b3 = T3[num4 >> 24];
			num4 = (uint)(num8 | (b << 8) | (b2 << 16) | (b3 << 24));
			byte num9 = T0[num7 & 0xFF];
			byte b4 = T1[(num7 >> 8) & 0xFF];
			byte b5 = T2[(num7 >> 16) & 0xFF];
			byte b6 = T3[num7 >> 24];
			num7 = (uint)(num9 | (b4 << 8) | (b5 << 16) | (b6 << 24));
			num = num4 | ((ulong)num7 << 32);
			byte num10 = T0[num6 & 0xFF];
			byte b7 = T1[(num6 >> 8) & 0xFF];
			byte b8 = T2[(num6 >> 16) & 0xFF];
			byte b9 = T3[num6 >> 24];
			num6 = (uint)(num10 | (b7 << 8) | (b8 << 16) | (b9 << 24));
			byte num11 = T0[num5 & 0xFF];
			byte b10 = T1[(num5 >> 8) & 0xFF];
			byte b11 = T2[(num5 >> 16) & 0xFF];
			byte b12 = T3[num5 >> 24];
			num5 = (uint)(num11 | (b10 << 8) | (b11 << 16) | (b12 << 24));
	...
	private void EncryptBlock_128(byte[] input, int inOff, byte[] output, int outOff)
	{
		...
		while (true)
		{
			...
			uint num6 = (uint)num2;
			uint num7 = (uint)(num2 >> 32);
			byte num8 = S0[num4 & 0xFF];
			byte b = S1[(num4 >> 8) & 0xFF];
			byte b2 = S2[(num4 >> 16) & 0xFF];
			byte b3 = S3[num4 >> 24];
			num4 = (uint)(num8 | (b << 8) | (b2 << 16) | (b3 << 24));
			byte num9 = S0[num7 & 0xFF];
			byte b4 = S1[(num7 >> 8) & 0xFF];
			byte b5 = S2[(num7 >> 16) & 0xFF];
			byte b6 = S3[num7 >> 24];
			num7 = (uint)(num9 | (b4 << 8) | (b5 << 16) | (b6 << 24));
			num = num4 | ((ulong)num7 << 32);
			byte num10 = S0[num6 & 0xFF];
			byte b7 = S1[(num6 >> 8) & 0xFF];
			byte b8 = S2[(num6 >> 16) & 0xFF];
			byte b9 = S3[num6 >> 24];
			num6 = (uint)(num10 | (b7 << 8) | (b8 << 16) | (b9 << 24));
			byte num11 = S0[num5 & 0xFF];
			byte b10 = S1[(num5 >> 8) & 0xFF];
			byte b11 = S2[(num5 >> 16) & 0xFF];
			byte b12 = S3[num5 >> 24];
			num5 = (uint)(num11 | (b10 << 8) | (b11 << 16) | (b12 << 24));
			num2 = num6 | ((ulong)num5 << 32);
			num = MixColumn(num);
	...
	private void SubBytes()
	{
		for (int i = 0; i < wordsInBlock; i++)
		{
			...
			byte num4 = S0[num2 & 0xFF];
			byte b = S1[(num2 >> 8) & 0xFF];
			byte b2 = S2[(num2 >> 16) & 0xFF];
			byte b3 = S3[num2 >> 24];
			num2 = (uint)(num4 | (b << 8) | (b2 << 16) | (b3 << 24));
			byte num5 = S0[num3 & 0xFF];
			byte b4 = S1[(num3 >> 8) & 0xFF];
			byte b5 = S2[(num3 >> 16) & 0xFF];
			byte b6 = S3[num3 >> 24];
	...
	private void InvSubBytes()
	{
		for (int i = 0; i < wordsInBlock; i++)
		{
			...
			uint num2 = (uint)num;
			uint num3 = (uint)(num >> 32);
			byte num4 = T0[num2 & 0xFF];
			byte b = T1[(num2 >> 8) & 0xFF];
			byte b2 = T2[(num2 >> 16) & 0xFF];
			byte b3 = T3[num2 >> 24];
			num2 = (uint)(num4 | (b << 8) | (b2 << 16) | (b3 << 24));
			byte num5 = T0[num3 & 0xFF];
			byte b4 = T1[(num3 >> 8) & 0xFF];
			byte b5 = T2[(num3 >> 16) & 0xFF];
			byte b6 = T3[num3 >> 24];
			num3 = (uint)(num5 | (b4 << 8) | (b5 << 16) | (b6 << 24));
			internalState[i] = num2 | ((ulong)num3 << 32);
		}
	...
	private void ShiftRows()
	{
		...
		case 4:
		{
			...
			num10 ^= num14;
			num12 ^= num14;
			num14 = (num11 ^ num13) & 0xFFFFFFFF0000L;
			num11 ^= num14;
			num13 ^= num14;
		...
		case 8:
		{
			...
			num ^= num9;
			num5 ^= num9;
			num9 = (num2 ^ num6) & 0xFFFFFFFF000000L;
			num2 ^= num9;
			num6 ^= num9;
			num9 = (num3 ^ num7) & 0xFFFFFFFF0000L;
			num3 ^= num9;
			num7 ^= num9;
			num9 = (num4 ^ num8) & 0xFFFFFFFF00L;
			num4 ^= num9;
			num8 ^= num9;
			num9 = (num ^ num3) & 0xFFFF0000FFFF0000uL;
			num ^= num9;
			num3 ^= num9;
			num9 = (num2 ^ num4) & 0xFFFF0000FFFF00L;
			num2 ^= num9;
			num4 ^= num9;
			num9 = (num5 ^ num7) & 0xFFFF0000FFFF0000uL;
			num5 ^= num9;
			num7 ^= num9;
			num9 = (num6 ^ num8) & 0xFFFF0000FFFF00L;
			num6 ^= num9;
			num8 ^= num9;
