...
public class CmsSignedDataStreamGenerator : CmsSignedGenerator
{
		...
		public SignerInfo Generate(DerObjectIdentifier contentType, AlgorithmIdentifier digestAlgorithm, byte[] calculatedDigest)
		{
			try
			{
				...
				if (_sAttr != null)
				{
					...
					BestHTTP.SecureProtocol.Org.BouncyCastle.Asn1.Cms.AttributeTable attributeTable = _sAttr.GetAttributes(baseParameters);
				...
				if (_unsAttr != null)
				{
					...
					BestHTTP.SecureProtocol.Org.BouncyCastle.Asn1.Cms.AttributeTable attributes = _unsAttr.GetAttributes(baseParameters2);
	...
	private class CmsSignedDataOutputStream : BaseOutputStream
	{
		...
		private void DoClose()
		{
			...
			foreach (DictionaryEntry messageDigest in outer._messageDigests)
			{
				outer._messageHashes.Add(messageDigest.Key, DigestUtilities.DoFinal((IDigest)messageDigest.Value));
			}
