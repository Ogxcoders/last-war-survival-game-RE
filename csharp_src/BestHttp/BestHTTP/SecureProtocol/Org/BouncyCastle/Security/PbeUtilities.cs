...
public sealed class PbeUtilities
{
	...
	public static ICipherParameters GenerateCipherParameters(string algorithm, char[] password, bool wrongPkcs12Zero, Asn1Encodable pbeParameters)
	{
		...
		if (IsPkcs5Scheme2(text))
		{
			...
			parameters = MakePbeGenerator((string)algorithmType[text], null, array, salt, iterationCount).GenerateDerivedParameters(algorithm2.Id, keySize);
		...
		{
			PbeParametersGenerator pbeParametersGenerator = MakePbeGenerator((string)algorithmType[text], new Sha1Digest(), array, salt, iterationCount);
		...
		{
			PbeParametersGenerator pbeParametersGenerator2 = MakePbeGenerator((string)algorithmType[text], new Sha256Digest(), array, salt, iterationCount);
		...
		{
			PbeParametersGenerator pbeParametersGenerator3 = MakePbeGenerator((string)algorithmType[text], new MD5Digest(), array, salt, iterationCount);
		...
		{
			PbeParametersGenerator pbeParametersGenerator4 = MakePbeGenerator((string)algorithmType[text], new MD2Digest(), array, salt, iterationCount);
		...
		else if (Platform.StartsWith(text, "PBEwithHmac"))
		{
			...
			PbeParametersGenerator pbeParametersGenerator5 = MakePbeGenerator((string)algorithmType[text], digest, array, salt, iterationCount);
