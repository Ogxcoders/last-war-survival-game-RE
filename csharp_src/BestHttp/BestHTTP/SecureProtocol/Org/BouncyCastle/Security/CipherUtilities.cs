...
public sealed class CipherUtilities
{
	...
	static CipherUtilities()
	{
		...
		((CipherAlgorithm)(object)Enums.GetArbitraryValue(typeof(CipherAlgorithm))/*cast due to .constrained prefix*/).ToString();
		((CipherMode)(object)Enums.GetArbitraryValue(typeof(CipherMode))/*cast due to .constrained prefix*/).ToString();
		((CipherPadding)(object)Enums.GetArbitraryValue(typeof(CipherPadding))/*cast due to .constrained prefix*/).ToString();
	...
	public static IBufferedCipher GetCipher(string algorithm)
	{
		...
		try
		{
			cipherAlgorithm = (CipherAlgorithm)(object)Enums.GetEnumValue(typeof(CipherAlgorithm), text2);
		}
		...
		if (array.Length > 2)
		{
			...
			else
			{
				try
				{
					cipherPadding = (CipherPadding)(object)Enums.GetEnumValue(typeof(CipherPadding), text4);
				}
				catch (ArgumentException)
				{
					throw new SecurityUtilityException("Cipher " + algorithm + " not recognised.");
		...
		if (array.Length > 1)
		{
			...
			try
			{
				switch ((text6 == "") ? CipherMode.NONE : ((CipherMode)(object)Enums.GetEnumValue(typeof(CipherMode), text6)))
				{
				case CipherMode.CBC:
