...
public class TspUtil
{
	...
	public static ICollection GetSignatureTimestamps(SignerInformation signerInfo)
	{
		...
		{
			foreach (BestHTTP.SecureProtocol.Org.BouncyCastle.Asn1.Cms.Attribute item in unsignedAttributes.GetAll(PkcsObjectIdentifiers.IdAASignatureTimeStampToken))
			{
				foreach (Asn1Encodable attrValue in item.AttrValues)
				{
					try
					{
						TimeStampToken timeStampToken = new TimeStampToken(BestHTTP.SecureProtocol.Org.BouncyCastle.Asn1.Cms.ContentInfo.GetInstance(attrValue.ToAsn1Object()));
