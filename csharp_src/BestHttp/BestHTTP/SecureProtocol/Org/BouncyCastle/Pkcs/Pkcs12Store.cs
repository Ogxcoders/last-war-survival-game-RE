...
public class Pkcs12Store
{
	...
	public void Load(Stream input, char[] password)
	{
		...
		if (password != null && pfx.MacData != null)
		{
			...
			if (!Arrays.ConstantTimeAreEqual(a, digest))
			{
				...
				if (!Arrays.ConstantTimeAreEqual(CalculatePbeMac(algorithmID.Algorithm, salt, intValue, password, wrongPkcs12Zero: true, octets), digest))
				{
					throw new IOException("PKCS12 key store MAC invalid - wrong password or corrupted file.");
				}
		...
		if (authSafe.ContentType.Equals(PkcsObjectIdentifiers.Data))
		{
			...
			foreach (ContentInfo contentInfo2 in contentInfo)
			{
				...
				else if (contentType.Equals(PkcsObjectIdentifiers.EncryptedData) && password != null)
				{
					...
					array = CryptPbeData(forEncryption: false, instance.EncryptionAlgorithm, password, wrongPkcs12Zero, instance.Content.GetOctets());
				}
				if (array == null)
				{
					continue;
		...
		{
			byte[] octets2 = ((Asn1OctetString)new CertBag((Asn1Sequence)item2.BagValue).CertValue).GetOctets();
	...
	{
		IBufferedCipher obj = (PbeUtilities.CreateEngine(algId.Algorithm) as IBufferedCipher) ?? throw new Exception("Unknown encryption algorithm: " + algId.Algorithm);
		ICipherParameters parameters = PbeUtilities.GenerateCipherParameters(pbeParameters: Pkcs12PbeParams.GetInstance(algId.Parameters), algorithmOid: algId.Algorithm, password: password, wrongPkcs12Zero: wrongPkcs12Zero);
