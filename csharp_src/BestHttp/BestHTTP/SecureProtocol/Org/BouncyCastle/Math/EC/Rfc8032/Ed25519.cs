...
public abstract class Ed25519
{
	...
	private static sbyte[] GetWnaf(uint[] n, int width)
	{
		...
		while (num10 < array.Length)
		{
			...
			while (num9 < 16)
			{
				...
				uint num13 = (num12 & num6) + num8;
				num8 = num13 & num7;
				num13 -= num8 << 1;
				num8 >>= width - 1;
				array2[(num10 << 4) + num9] = (sbyte)num13;
				num9 += width;
			}
			num10++;
			num9 -= 16;
	...
	private static void ScalarMultBase(byte[] k, PointAccum r)
	{
		...
		while (true)
		{
			for (int j = 0; j < 8; j++)
			{
				...
				int index = (int)((num2 ^ (uint)(-num3)) & 7);
	...
	private static void ScalarMultStraussVar(uint[] nb, uint[] np, PointExt p, PointAccum r)
	{
		...
		sbyte[] wnaf2 = GetWnaf(np, num);
		PointExt[] array = PointPrecompVar(p, 1 << num - 2);
		PointSetNeutral(r);
		int num2 = 255;
