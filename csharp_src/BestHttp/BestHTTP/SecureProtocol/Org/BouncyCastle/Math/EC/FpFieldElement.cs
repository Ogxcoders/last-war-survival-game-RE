...
public class FpFieldElement : AbstractFpFieldElement
{
	...
	public override ECFieldElement Sqrt()
	{
		...
		if (q.TestBit(2))
		{
			...
			if (ModMult(x, bigInteger).Equals(BigInteger.One))
			{
				return CheckSqrt(new FpFieldElement(q, r, x));
			}
			BigInteger x2 = BigInteger.Two.ModPow(q.ShiftRight(2), q);
			BigInteger bigInteger2 = ModMult(x, x2);
			return CheckSqrt(new FpFieldElement(q, r, bigInteger2));
		}
		BigInteger bigInteger3 = q.ShiftRight(1);
		if (!this.x.ModPow(bigInteger3, q).Equals(BigInteger.One))
		{
			return null;
		}
		BigInteger bigInteger4 = this.x;
		BigInteger bigInteger5 = ModDouble(ModDouble(bigInteger4));
		BigInteger k = bigInteger3.Add(BigInteger.One);
		BigInteger obj = q.Subtract(BigInteger.One);
		while (true)
		{
			BigInteger bigInteger6 = BigInteger.Arbitrary(q.BitLength);
			if (bigInteger6.CompareTo(q) < 0 && ModReduce(bigInteger6.Multiply(bigInteger6).Subtract(bigInteger5)).ModPow(bigInteger3, q).Equals(obj))
			{
				BigInteger[] array = LucasSequence(bigInteger6, bigInteger4, k);
				BigInteger bigInteger7 = array[0];
				BigInteger bigInteger8 = array[1];
				if (ModMult(bigInteger8, bigInteger8).Equals(bigInteger5))
				{
					return new FpFieldElement(q, r, ModHalfAbs(bigInteger8));
				}
	...
	protected virtual BigInteger ModReduce(BigInteger x)
	{
		...
		else
		{
			...
			else
			{
				...
				BigInteger bigInteger2 = r.Negate().Multiply(x.ShiftRight(bitLength - num)).ShiftRight(bitLength + num)
					.Multiply(q);
