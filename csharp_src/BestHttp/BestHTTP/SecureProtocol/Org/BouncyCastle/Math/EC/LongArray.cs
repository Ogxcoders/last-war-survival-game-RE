...
internal class LongArray
{
	...
	public BigInteger ToBigInteger()
	{
		...
		{
			long num5 = m_ints[num4];
			for (int num6 = 7; num6 >= 0; num6--)
			{
				array2[num2++] = (byte)((ulong)num5 >> 8 * num6);
			}
		}
	...
	public LongArray ModMultiplyLD(LongArray other, int m, int[] ks)
	{
		...
		for (int num11 = 56; num11 >= 0; num11 -= 8)
		{
			...
			{
				int num12 = (int)(ints[j] >>> num11);
				int num13 = num12 & num10;
				int num14 = (num12 >>> 4) & num10;
		...
		for (int num15 = 56; num15 >= 0; num15 -= 8)
		{
			...
			{
				int num16 = (int)(ints[k] >>> num15);
				int num17 = num16 & num10;
				int num18 = (num16 >>> 4) & num10;
	...
	public LongArray ModMultiplyAlt(LongArray other, int m, int[] ks)
	{
		...
		while (--num25 > 1)
		{
			if (((ulong)num25 & 1uL) == 0L)
			{
				AddShiftedUp(array3, array2[num25 >>> 1], array3, array2[num25], num6, num9);
			}
			else
			{
	...
	public LongArray ModSquare(int m, int[] ks)
	{
		...
		while (num2 < num)
		{
			long num3 = m_ints[num2 >>> 1];
			array[num2++] = Interleave2_32to64((int)num3);
			array[num2++] = Interleave2_32to64((int)(num3 >>> 32));
		}
	...
	private static void SquareInPlace(long[] x, int xLen, int m, int[] ks)
	{
		...
		{
			long num2 = x[xLen];
			x[--num] = Interleave2_32to64((int)(num2 >>> 32));
	...
	{
		return (x & long.MinValue) | Interleave3_21to63((int)x & 0x1FFFFF) | (Interleave3_21to63((int)(x >>> 21) & 0x1FFFFF) << 1) | (Interleave3_21to63((int)(x >>> 42) & 0x1FFFFF) << 2);
	}
	...
	private static long Interleave3_21to63(int x)
	{
		...
		return ((INTERLEAVE3_TABLE[x >>> 14] & 0xFFFFFFFFu) << 42) | ((num2 & 0xFFFFFFFFu) << 21) | (num & 0xFFFFFFFFu);
	}
	...
	{
		return Interleave3_13to65((int)x & 0x1FFF) | (Interleave3_13to65((int)(x >>> 13) & 0x1FFF) << 1) | (Interleave3_13to65((int)(x >>> 26) & 0x1FFF) << 2) | (Interleave3_13to65((int)(x >>> 39) & 0x1FFF) << 3) | (Interleave3_13to65((int)(x >>> 52) & 0x1FFF) << 4);
	}

	private static long Interleave3_13to65(int x)
	{
		int num = INTERLEAVE5_TABLE[x & 0x7F];
		return ((INTERLEAVE5_TABLE[x >>> 7] & 0xFFFFFFFFu) << 35) | (num & 0xFFFFFFFFu);
	}
	...
	{
		return (x & long.MinValue) | INTERLEAVE7_TABLE[(int)x & 0x1FF] | (INTERLEAVE7_TABLE[(int)(x >>> 9) & 0x1FF] << 1) | (INTERLEAVE7_TABLE[(int)(x >>> 18) & 0x1FF] << 2) | (INTERLEAVE7_TABLE[(int)(x >>> 27) & 0x1FF] << 3) | (INTERLEAVE7_TABLE[(int)(x >>> 36) & 0x1FF] << 4) | (INTERLEAVE7_TABLE[(int)(x >>> 45) & 0x1FF] << 5) | (INTERLEAVE7_TABLE[(int)(x >>> 54) & 0x1FF] << 6);
	}
	...
	private static long Interleave4_16to64(int x)
	{
		...
		return ((INTERLEAVE4_TABLE[x >>> 8] & 0xFFFFFFFFu) << 32) | (num & 0xFFFFFFFFu);
	}

	private static long Interleave2_32to64(int x)
	{
		int num = INTERLEAVE2_TABLE[x & 0xFF] | (INTERLEAVE2_TABLE[(x >>> 8) & 0xFF] << 16);
		return (((INTERLEAVE2_TABLE[(x >>> 16) & 0xFF] | (INTERLEAVE2_TABLE[x >>> 24] << 16)) & 0xFFFFFFFFu) << 32) | (num & 0xFFFFFFFFu);
	}
