...
internal class SecT571K1Curve : AbstractF2mCurve
{
		...
		public virtual ECPoint Lookup(int index)
		{
			...
			for (int i = 0; i < m_size; i++)
			{
				...
				for (int j = 0; j < 9; j++)
				{
					...
					array2[j] ^= m_table[num + 9 + j] & num2;
				}
				num += 18;
			}
			return m_outer.CreateRawPoint(new SecT571FieldElement(array), new SecT571FieldElement(array2), withCompression: false);
		}
	}
	...
	public override ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			Nat576.Copy64(((SecT571FieldElement)obj.RawXCoord).x, 0, array, num);
			num += 9;
			Nat576.Copy64(((SecT571FieldElement)obj.RawYCoord).x, 0, array, num);
			num += 9;
		}
