...
public abstract class Primes
{
	...
	private static STOutput ImplSTRandomPrime(IDigest d, int length, byte[] primeSeed)
	{
		...
		int num6 = num3;
		BigInteger bigInteger = HashGen(d, primeSeed, num5 + 1).Mod(One.ShiftLeft(length - 1)).SetBit(length - 1);
		...
		while (true)
		{
			...
			{
				BigInteger bigInteger5 = HashGen(d, primeSeed, num5 + 1).Mod(bigInteger4.Subtract(Three)).Add(Two);
