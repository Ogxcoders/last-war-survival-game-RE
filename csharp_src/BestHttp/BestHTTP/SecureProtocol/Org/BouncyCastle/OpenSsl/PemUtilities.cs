...
internal sealed class PemUtilities
{
	...
	static PemUtilities()
	{
		((PemBaseAlg)(object)Enums.GetArbitraryValue(typeof(PemBaseAlg))/*cast due to .constrained prefix*/).ToString();
		((PemMode)(object)Enums.GetArbitraryValue(typeof(PemMode))/*cast due to .constrained prefix*/).ToString();
	}
	...
	private static void ParseDekAlgName(string dekAlgName, out PemBaseAlg baseAlg, out PemMode mode)
	{
		try
		{
			...
			{
				baseAlg = (PemBaseAlg)(object)Enums.GetEnumValue(typeof(PemBaseAlg), dekAlgName);
				return;
			}
			int num = dekAlgName.LastIndexOf('-');
			if (num >= 0)
			{
				baseAlg = (PemBaseAlg)(object)Enums.GetEnumValue(typeof(PemBaseAlg), dekAlgName.Substring(0, num));
				mode = (PemMode)(object)Enums.GetEnumValue(typeof(PemMode), dekAlgName.Substring(num + 1));
				return;
			}
		}
	...
	internal static byte[] Crypt(bool encrypt, byte[] bytes, char[] password, string dekAlgName, byte[] iv)
	{
		...
		IBufferedCipher cipher = CipherUtilities.GetCipher(string.Concat(text2, "/", mode, "/", text));
