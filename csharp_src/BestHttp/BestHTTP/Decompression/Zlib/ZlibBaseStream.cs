...
internal class ZlibBaseStream : Stream
{
	...
	private ZlibCodec z
	{
		get
		{
			if (_z == null)
			{
				...
				{
					_z.InitializeInflate(windowBitsMax, flag);
				}
				else
				{
					_z.Strategy = Strategy;
					_z.InitializeDeflate(_level, windowBitsMax, flag);
				}
			}
			return _z;
		}
	...
	private byte[] workingBuffer
	{
		get
		{
			...
			{
				_workingBuffer = VariableSizedBufferPool.Get(_bufferSize, canBeLarger: true);
			}
	...
	public ZlibBaseStream(Stream stream, CompressionMode compressionMode, CompressionLevel level, ZlibStreamFlavor flavor, bool leaveOpen)
		: this(stream, compressionMode, level, flavor, leaveOpen, 15)
	{
	}
	...
	private void finish()
	{
		...
		else
		{
			...
			byte[] array = VariableSizedBufferPool.Get(8L, canBeLarger: true);
			...
			{
				Array.Copy(_z.InputBuffer, _z.NextIn, array, 0, 8);
			}
	...
	private void end()
	{
		if (z != null)
		{
			...
			_z = null;
			VariableSizedBufferPool.Release(_workingBuffer);
			_workingBuffer = null;
		}
	}
	...
	{
		int num = 0;
		byte[] array = VariableSizedBufferPool.Get(10L, canBeLarger: true);
		int num2 = _stream.Read(array, 0, 10);
		switch (num2)
		{
		case 0:
			VariableSizedBufferPool.Release(array);
		...
		case 10:
		{
			...
			if ((array[3] & 4) == 4)
			{
				...
				byte[] buffer = VariableSizedBufferPool.Get(num4, canBeLarger: true);
				num2 = _stream.Read(buffer, 0, num4);
				if (num2 != num4)
				{
