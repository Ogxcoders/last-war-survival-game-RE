...
internal sealed class DeflateManager
{
	...
	internal int Initialize(ZlibCodec codec, CompressionLevel level, int windowBits, int memLevel, CompressionStrategy strategy)
	{
		...
		window = VariableSizedBufferPool.Get(w_size * 2, canBeLarger: false);
		prev = new short[w_size];
		head = new short[hash_size];
		lit_bufsize = 1 << memLevel + 6;
		pending = VariableSizedBufferPool.Get(lit_bufsize * 4, canBeLarger: false);
	...
	internal int End()
	{
		...
		VariableSizedBufferPool.Release(pending);
		pending = null;
		head = null;
		prev = null;
		VariableSizedBufferPool.Release(window);
		window = null;
