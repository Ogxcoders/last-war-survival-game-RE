...
public sealed class Packet
{
	...
	public bool HasAllAttachment
	{
		get
		{
			if (Attachments != null)
			{
				return Attachments.Count == AttachmentCount;
			}
			return false;
		}
	}
	...
	{
		int i = 0;
		TransportEvent = (TransportEventTypes)ToInt(from[i++]);
		if (from.Length > i && ToInt(from[i]) >= 0)
		{
			SocketIOEvent = (SocketIOEventTypes)ToInt(from[i++]);
		}
		else
		{
			SocketIOEvent = SocketIOEventTypes.Unknown;
		...
		{
			int num3 = i++;
			for (; from.Length > i && ToInt(from[i]) >= 0; i++)
			{
			}
			int result2 = 0;
	...
	{
		return new Packet(TransportEvent, SocketIOEvent, Namespace, Payload, 0, Id)
		{
			EventName = EventName,
