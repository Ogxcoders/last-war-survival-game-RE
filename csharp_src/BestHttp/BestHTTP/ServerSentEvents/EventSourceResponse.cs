...
public sealed class EventSourceResponse : HTTPResponse, IProtocol
{
	...
	private new void ReadChunked(Stream stream)
	{
		...
		byte[] buffer = VariableSizedBufferPool.Get(num, canBeLarger: true);
		while (num != 0)
		{
	...
	public void FeedData(byte[] buffer, int count)
	{
		...
		if (LineBuffer == null)
		{
			LineBuffer = VariableSizedBufferPool.Get(1024L, canBeLarger: true);
		}
		int num = 0;
		int num2;
		do
		{
			...
			if (LineBuffer.Length < LineBufferPos + (num4 - num))
			{
				...
				VariableSizedBufferPool.Resize(ref LineBuffer, newSize, canBeLarger: true);
			}
