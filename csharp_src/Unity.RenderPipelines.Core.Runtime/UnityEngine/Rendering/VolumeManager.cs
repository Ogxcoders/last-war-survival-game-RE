...
public sealed class VolumeManager
{
	...
	public VolumeStack CreateStack()
	{
		...
		volumeStack.Reload(baseComponentTypes);
		return volumeStack;
	}
	...
	private void ReloadBaseTypes()
	{
		...
		baseComponentTypes = from t in CoreUtils.GetAllTypesDerivedFrom<VolumeComponent>()
			where !t.IsAbstract
			select t;
		foreach (Type baseComponentType in baseComponentTypes)
		{
			VolumeComponent item = (VolumeComponent)ScriptableObject.CreateInstance(baseComponentType);
