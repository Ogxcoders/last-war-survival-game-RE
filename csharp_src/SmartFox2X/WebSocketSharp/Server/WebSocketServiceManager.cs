...
public class WebSocketServiceManager
{
	...
	internal void Stop(CloseEventArgs e, bool send, bool wait)
	{
		lock (_sync)
		{
			...
			byte[] frameAsBytes = (send ? WebSocketFrame.CreateCloseFrame(e.PayloadData, mask: false).ToByteArray() : null);
			TimeSpan timeout = (wait ? _waitTime : TimeSpan.Zero);
			foreach (WebSocketServiceHost value in _hosts.Values)
			{
	...
	{
		string text = _state.CheckIfStart() ?? stream.CheckIfCanRead() ?? ((length < 1) ? "'length' is less than 1." : null);
		if (text != null)
		{
			_logger.Error(text);
			return;
