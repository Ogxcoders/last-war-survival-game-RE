...
public abstract class WebSocketServiceHost
{
	...
	internal void Stop(ushort code, string reason)
	{
		...
		bool num = !code.IsReserved();
		byte[] frameAsBytes = (num ? WebSocketFrame.CreateCloseFrame(e.PayloadData, mask: false).ToByteArray() : null);
		TimeSpan timeout = (num ? WaitTime : TimeSpan.Zero);
		Sessions.Stop(e, frameAsBytes, timeout);
	}
