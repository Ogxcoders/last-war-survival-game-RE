...
internal sealed class EndPointManager
{
	...
	private static void addPrefix(string uriPrefix, HttpListener listener)
	{
		...
		getEndPointListener(httpListenerPrefix.Host, httpListenerPrefix.Port, listener, httpListenerPrefix.IsSecure).AddPrefix(httpListenerPrefix, listener);
	}
	...
	private static IPAddress convertToAddress(string hostname)
	{
		...
		try
		{
			...
			return (hostEntry != null) ? hostEntry.AddressList[0] : IPAddress.Any;
		}
		catch
		{
			return IPAddress.Any;
	...
	private static EndPointListener getEndPointListener(string host, int port, HttpListener listener, bool secure)
	{
		...
		{
			return dictionary[port] = new EndPointListener(iPAddress, port, secure, listener.CertificateFolderPath, listener.SslConfiguration, listener.ReuseAddress);
		}
	...
	private static void removePrefix(string uriPrefix, HttpListener listener)
	{
		...
		{
			getEndPointListener(httpListenerPrefix.Host, httpListenerPrefix.Port, listener, httpListenerPrefix.IsSecure).RemovePrefix(httpListenerPrefix, listener);
		}
	}
