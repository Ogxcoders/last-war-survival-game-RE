...
public class SFSIOHandler : IoHandler
{
	...
	private PacketReadState ReadState => (PacketReadState)fsm.GetCurrentState();
	...
	{
		log.Debug("Handling Header Size. Length: " + data.Length + " (" + (pendingPacket.Header.BigSized ? "big" : "small") + ")");
		int num = -1;
	...
	private ByteArray HandleDataSizeFragment(ByteArray data)
	{
		...
		int num = (pendingPacket.Header.BigSized ? (INT_BYTE_SIZE - pendingPacket.Buffer.Length) : (SHORT_BYTE_SIZE - pendingPacket.Buffer.Length));
		if (data.Length >= num)
		{
			...
			int expectedLength = (pendingPacket.Header.BigSized ? byteArray.ReadInt() : byteArray.ReadShort());
			log.Debug("DataSize is ready: " + expectedLength + " bytes");
			pendingPacket.Header.ExpectedLength = expectedLength;
