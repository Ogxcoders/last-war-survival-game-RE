...
public class SFSProtocolCodec : IProtocolCodec
{
	...
	private ISFSObject PrepareUDPPacket(IMessage message)
	{
		SFSObject sFSObject = new SFSObject();
		((ISFSObject)sFSObject).PutByte(CONTROLLER_ID, Convert.ToByte(message.TargetController));
		((ISFSObject)sFSObject).PutInt(USER_ID, (bitSwarm.Sfs.MySelf != null) ? bitSwarm.Sfs.MySelf.Id : (-1));
		((ISFSObject)sFSObject).PutLong(UDP_PACKET_ID, bitSwarm.NextUdpPacketId());
		((ISFSObject)sFSObject).PutSFSObject(PARAM_ID, message.Content);
		return sFSObject;
	}
	...
	private void DispatchRequest(ISFSObject requestObject)
	{
		...
		(bitSwarm.GetController(id) ?? throw new SFSError("Cannot handle server response. Unknown controller, id: " + id)).HandleMessage(message);
	}
}
