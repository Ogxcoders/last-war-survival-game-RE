...
public class BoundingBoxFollower : MonoBehaviour
{
	...
	public void Initialize(bool overwrite = false)
	{
		...
		if (base.gameObject.activeInHierarchy)
		{
			...
			{
				AddCollidersForSkin(skin, index, components, ref collidersCount);
			}
			if (skeleton.Skin != null)
			{
				AddCollidersForSkin(skeleton.Skin, index, components, ref collidersCount);
			}
		}
	...
	private void MatchAttachment(Attachment attachment)
	{
		...
		if (DebugMessages)
		{
			Debug.LogFormat("Collider for BoundingBoxAttachment named '{0}' was not initialized. It is possibly from a new skin. currentAttachmentName will be null. You may need to call BoundingBoxFollower.Initialize(overwrite: true);", boundingBoxAttachment.Name);
		}
	}
}
