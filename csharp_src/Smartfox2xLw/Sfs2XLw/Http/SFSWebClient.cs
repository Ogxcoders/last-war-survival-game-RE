...
public class SFSWebClient
{
	...
	public void UploadValuesAsync(Uri uri, string paramName, string encodedData)
	{
		...
		{
			OnHttpResponse(error: true, "Http error creating http connection: " + ex.ToString());
			return;
		}
		try
		{
			...
			if (array3.Length < 2)
			{
				OnHttpResponse(error: true, "Error during http response: connection closed by remote side");
				return;
			}
			char[] trimChars = new char[1] { ' ' };
			string message = array3[1].TrimEnd(trimChars);
			OnHttpResponse(error: false, message);
		}
		catch (Exception ex2)
		{
			OnHttpResponse(error: true, "Error during http request: " + ex2.ToString() + " " + ex2.StackTrace);
		}
		finally
		{
			try
			...
			{
				OnHttpResponse(error: true, "Error during http scocket shutdown: " + ex3.ToString() + " " + ex3.StackTrace);
			}
		}
	}
}
