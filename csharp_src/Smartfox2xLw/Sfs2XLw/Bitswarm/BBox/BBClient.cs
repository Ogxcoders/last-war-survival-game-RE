...
public class BBClient : IDispatchable
{
	...
	public int PollSpeed
	{
		...
		set
		{
			pollSpeed = ((value >= 50 && value <= 5000) ? value : 300);
		}
	}
	...
	internal void OnHttpResponse(bool error, string response)
	{
		...
		try
		{
			...
			case "err01":
			{
				...
				HandleConnectionLost(fireEvent: false);
	...
	private void SendRequest(string cmd, object data)
	{
		...
		GetWebClient().UploadValuesAsync(new Uri(bbUrl), "sfsHttp", encodedData);
	}
	...
	private string EncodeRequest(string cmd, object data)
	{
		...
		return ((sessId == null) ? "null" : sessId) + Convert.ToString('|') + cmd + Convert.ToString('|') + text;
	}
