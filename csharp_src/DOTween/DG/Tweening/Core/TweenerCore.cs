...
public class TweenerCore<T1, T2, TPlugOptions> : Tweener where TPlugOptions : struct, IPlugOptions
{
	...
	public override Tweener ChangeStartValue(object newStartValue, float newDuration = -1f)
	{
		...
		if ((object)type != typeofT2)
		{
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning("ChangeStartValue: incorrect newStartValue type (is " + type?.ToString() + ", should be " + typeofT2?.ToString() + ")", this);
			}
			return this;
		}
	...
	public override Tweener ChangeEndValue(object newEndValue, float newDuration = -1f, bool snapStartValue = false)
	{
		...
		if ((object)type != typeofT2)
		{
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning("ChangeEndValue: incorrect newEndValue type (is " + type?.ToString() + ", should be " + typeofT2?.ToString() + ")", this);
			}
			return this;
		}
	...
	public override Tweener ChangeValues(object newStartValue, object newEndValue, float newDuration = -1f)
	{
		...
		if ((object)type != typeofT2)
		{
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning("ChangeValues: incorrect value type (is " + type?.ToString() + ", should be " + typeofT2?.ToString() + ")", this);
			}
			return this;
		}
		if ((object)type2 != typeofT2)
		{
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning("ChangeValues: incorrect value type (is " + type2?.ToString() + ", should be " + typeofT2?.ToString() + ")", this);
			}
			return this;
		}
