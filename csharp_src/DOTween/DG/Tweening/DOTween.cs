...
public class DOTween
{
	...
	public static void ManualUpdate(float deltaTime, float unscaledDeltaTime)
	{
		...
		{
			TweenManager.Update(UpdateType.Manual, deltaTime * timeScale, unscaledDeltaTime * timeScale);
		}
	}
	...
	{
		return Shake(getter, setter, duration, new Vector3(strength, strength, strength), vibrato, randomness, ignoreZAxis, vectorBased: false, fadeOut);
	}
	...
	private static TweenerCore<Vector3, Vector3[], Vector3ArrayOptions> Shake(DOGetter<Vector3> getter, DOSetter<Vector3> setter, float duration, Vector3 strength, int vibrato, float randomness, bool ignoreZAxis, bool vectorBased, bool fadeOut)
	{
		...
		for (int k = 0; k < num2; k++)
		{
			if (k < num2 - 1)
			{
				...
				if (vectorBased)
				{
					Vector3 vector = Quaternion.AngleAxis(UnityEngine.Random.Range(0f - randomness, randomness), Vector3.up) * Utils.Vector3FromAngle(num8, num);
				...
				else
				{
					...
					{
						Quaternion quaternion = Quaternion.AngleAxis(UnityEngine.Random.Range(0f - randomness, randomness), Vector3.up);
