...
public class RemoteLoggerTarget_UnityWebRequest : IRemoteLoggerTarget, ILoggerTarget
{
	private static RemoteLoggerTarget_UnityWebRequest _instance;
	...
	private ConcurrentQueue<string> _serializedQueue;
	...
	public static RemoteLoggerTarget_UnityWebRequest instance => _instance;
	...
	{
		_instance = new RemoteLoggerTarget_UnityWebRequest();
	...
	private void Initialize(string url, ClientInfo clientInfo)
	{
		...
		_serializedQueue = new ConcurrentQueue<string>();
	...
	{
		while (WorkThreadSerialize())
		{
		}
	}

	private bool WorkThreadSerialize()
	{
		...
		while (_sampleQueue.TryDequeue(out result))
		{
		...
		if (_finalLogs.Count > 0)
		{
			...
			try
			{
				string item = JsonConvert.SerializeObject(_finalLogContainer, _settings);
				_serializedQueue.Enqueue(item);
			}
			catch (Exception)
			{
			}
		...
		_consumeEvt.WaitOne(2000);
		return true;
	}

	private void HandleLogQueueList()
	{
		if (_closed)
		{
			return;
		...
		string result;
		while (_serializedQueue.TryDequeue(out result))
		{
			try
			{
				UnityWebRequest unityWebRequest = new UnityWebRequest(_url, "POST");
				byte[] bytes = Encoding.UTF8.GetBytes(result);
				unityWebRequest.uploadHandler = new UploadHandlerRaw(bytes);
				unityWebRequest.SetRequestHeader("x-log-apiversion", "0.6.0");
				unityWebRequest.SetRequestHeader("x-log-bodyrawsize", bytes.Length.ToString());
	...
	private void PushQueueHttps(LogLevel level, Dictionary<string, string> args, bool batch, string message = null)
	{
		...
		{
			remoteLogInfo.Add("resp", message);
		}
	...
	{
		if (reportLevel <= RemoteLoggerTarget.ReportLevel.Info && !CheckDuplicateMsg(message, "EMPTY"))
		{
			if (context is Dictionary<string, string> dictionary)
			{
			...
			{
				PushQueueHttps(LogLevel.Info, null, importance == LogImportance.Normal, message ?? "EMPTY");
			}
		}
	}
	...
	{
		if (reportLevel <= RemoteLoggerTarget.ReportLevel.Warning && !CheckDuplicateMsg(message, "EMPTY"))
		{
			if (context is Dictionary<string, string> dictionary)
			{
				dictionary["resp"] = message ?? "EMPTY";
			...
			{
				PushQueueHttps(LogLevel.Warning, null, importance == LogImportance.Normal, message ?? "EMPTY");
			}
		}
	}
	...
	{
		if (reportLevel <= RemoteLoggerTarget.ReportLevel.Error && !CheckDuplicateMsg(message, stackTrace))
		{
			if (context is Dictionary<string, string> dictionary)
			{
				dictionary["resp"] = message ?? "EMPTY";
			...
			{
				PushQueueHttps(LogLevel.Error, null, batch: false, message ?? "EMPTY");
			}
		}
	}
