...
internal class XmlSerializationReaderInterpreter : XmlSerializationReader
{
	...
	protected XmlTypeMapping GetTypeMap(Type type)
	{
		...
		throw new InvalidOperationException("Type " + type?.ToString() + " not mapped");
	}
			...
			while (base.Reader.NodeType != XmlNodeType.EndElement && base.Reader.ReadState == ReadState.Interactive)
			{
				if (base.Reader.IsStartElement(typeMap.ElementName, typeMap.Namespace) || _format == SerializationFormat.Encoded)
				{
					...
					if (!base.Reader.IsEmptyElement)
					{
						...
						ReadMembers((ClassMap)typeMap.ObjectMap, array, isValueList: true, readBySoapOrder: false);
						ReadEndElement();
						break;
					}
					base.Reader.Skip();
	...
	protected virtual object ReadClassInstance(XmlTypeMapping typeMap, bool isNullable, bool checkType)
	{
		...
		object obj = CreateInstance(typeMap.TypeData.Type, nonPublic: true);
	...
	{
		ReadMembers((ClassMap)typeMap.ObjectMap, ob, isValueList: false, readBySoapOrder: false);
	}
	...
	private void ReadMembers(ClassMap map, object ob, bool isValueList, bool readBySoapOrder)
	{
		...
		Fixup fixup = null;
		int num = -1;
		int num2 = ((!readBySoapOrder) ? int.MaxValue : ((map.ElementMembers == null) ? (-1) : map.ElementMembers.Count));
		...
		while (base.Reader.NodeType != XmlNodeType.EndElement && num < num2 - 1)
		{
			if (base.Reader.NodeType == XmlNodeType.Element)
			{
				...
				{
					xmlTypeMapElementInfo = ((!map.IsOrderDependentMap) ? map.GetElement(base.Reader.LocalName, base.Reader.NamespaceURI) : map.GetElement(base.Reader.LocalName, base.Reader.NamespaceURI, num));
				}
				else
				{
			...
			else if ((base.Reader.NodeType == XmlNodeType.Text || base.Reader.NodeType == XmlNodeType.CDATA) && map.XmlTextCollector != null)
			{
				if (map.XmlTextCollector is XmlTypeMapMemberExpandable)
				{
					...
					TypeData typeData = ((!(xmlTypeMapMemberExpandable2 is XmlTypeMapMemberFlatList xmlTypeMapMemberFlatList2)) ? xmlTypeMapMemberExpandable2.TypeData.ListItemTypeData : xmlTypeMapMemberFlatList2.ListMap.FindTextElement().TypeData);
					object value2 = ((typeData.Type == typeof(string)) ? base.Reader.ReadString() : ReadXmlNode(typeData, wrapped: false));
				...
				else
				{
					...
					{
						SetMemberValue(xmlTypeMapMemberElement, ob, base.Reader.ReadString(), isValueList);
					}
					else
					{
						SetMemberValue(xmlTypeMapMemberElement, ob, GetValueFromXmlString(base.Reader.ReadString(), xmlTypeMapElementInfo2.TypeData, xmlTypeMapElementInfo2.MappedType), isValueList);
					}
				}
			}
			else
	...
	private bool IsReadOnly(XmlTypeMapMember member, TypeData memType, object ob, bool isValueList)
	{
		...
		if (!member.IsReadOnly(ob.GetType()))
		{
			return !memType.HasPublicConstructor;
		}
		return true;
	...
	private object ReadObjectElement(XmlTypeMapElementInfo elem)
	{
		...
		{
			object obj = CreateInstance(elem.TypeData.Type, nonPublic: true);
	...
	private void AddListValue(TypeData listType, ref object list, int index, object value, bool canCreateInstance)
	{
		...
		if (list == null)
		{
			...
			list = CreateInstance(type, nonPublic: true);
		}
		type.GetMethod("Add", new Type[1] { listType.ListItemType }).Invoke(list, new object[1] { value });
	}
	...
	private object CreateList(Type listType)
	{
		...
		return CreateInstance(listType, nonPublic: true);
	}
	...
	private object InitializeList(TypeData listType)
	{
		...
		return CreateInstance(listType.Type, nonPublic: true);
	}
	...
	private object GetEnumValue(XmlTypeMapping typeMap, string val)
	{
		...
		return Enum.Parse(typeMap.TypeData.Type, enumName, ignoreCase: false);
	}
