...
public class XmlSchemaExporter
{
				...
				foreach (XmlTypeMapMemberElement item in elementMembers)
				{
					...
					if (xmlSchemaElement2 != null)
					{
						if (!xmlSchemaElement2.SchemaTypeName.Equals(xmlSchemaElement3.SchemaTypeName))
						{
							...
							text = text + "from namespace '" + schema3.TargetNamespace + "' references distinct types " + xmlSchemaElement3.SchemaTypeName.Name + " and " + xmlSchemaElement2.SchemaTypeName.Name + ". ";
	...
	{
		ClassMap classMap = (ClassMap)map.ObjectMap;
		if (classMap.XmlTextCollector != null)
		{
			if (map.BaseMap != null)
			{
				return DefinedInBaseMap(map.BaseMap, classMap.XmlTextCollector);
			}
			return false;
		}
		return false;
	...
	private XmlSchemaType GetSchemaXmlNodeType()
	{
		return new XmlSchemaComplexType
		{
			...
			{
				Items = { (XmlSchemaObject)new XmlSchemaAny() }
			}
		};
	}
	...
	private string ExportDefaultValue(TypeData typeData, XmlTypeMapping map, object defaultValue)
	{
		...
		{
			return ((EnumMap)map.ObjectMap).GetXmlName(map.TypeFullName, defaultValue);
		}
	...
	private XmlQualifiedName ExportArraySchema(XmlTypeMapping map, string defaultNamespace)
	{
		...
		if (encodedFormat)
		{
			...
			xmlAttribute.Value = ns + ((ns != "") ? ":" : "") + localName;
	...
	private string GetArrayKeyName(TypeData td)
	{
		...
		return "*arrayof*" + (listItemTypeData.IsListType ? GetArrayKeyName(listItemTypeData) : listItemTypeData.FullTypeName);
	}
