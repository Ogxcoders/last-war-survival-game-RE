...
internal class SerializationCodeGenerator
{
	...
	public static void Generate(string configFileName, string outputPath)
	{
		...
		foreach (SerializerInfo serializerInfo in serializers)
		{
			...
			try
			{
				XmlTypeMapping xmlMap = ((serializerInfo.SerializationFormat != SerializationFormat.Literal) ? new SoapReflectionImporter().ImportTypeMapping(type) : new XmlReflectionImporter().ImportTypeMapping(type));
	...
	public void GenerateContract(ArrayList generatedMaps)
	{
		...
		string text = ((_config == null || !_config.GenerateAsInternal) ? "public" : "internal");
		WriteLine("");
	...
	{
		Type type = ((xmlMap is XmlTypeMapping xmlTypeMapping) ? xmlTypeMapping.TypeData.Type : typeof(object[]));
		...
		if (elementMembers != null)
		{
			foreach (XmlTypeMapMemberElement item2 in elementMembers)
			{
				...
				else if (!(type2 == typeof(XmlTypeMapMemberAnyAttribute)))
				{
					...
					{
						XmlTypeMapElementInfo xmlTypeMapElementInfo = (XmlTypeMapElementInfo)item2.ElementInfo[0];
						GenerateWriteMemberElement(xmlTypeMapElementInfo, GetCast(xmlTypeMapElementInfo.TypeData, item2.TypeData, text5));
					}
					...
					else
					{
						...
						{
							WriteLineInd((flag ? "" : "else ") + "if (" + text5 + " is " + item4.TypeData.CSharpFullName + ") {");
	...
	private void GenerateWriteAnyElementContent(XmlTypeMapMemberAnyElement member, string memberValue)
	{
		...
		{
			text = GetObTempVar();
			WriteLineInd("foreach (object " + text + " in " + memberValue + ") {");
		}
		WriteLine("XmlNode " + obTempVar + " = " + text + " as XmlNode;");
		WriteLine("if (" + obTempVar + " == null && " + text + "!= null) throw new InvalidOperationException (\"A member with XmlAnyElementAttribute can only serialize and deserialize certain XmlNode types.\");");
		string obTempVar2 = GetObTempVar();
		WriteLine("XmlNode " + obTempVar2 + " = " + obTempVar + ";");
		WriteLineInd("if (" + obTempVar2 + " is XmlElement) {");
		if (!member.IsDefaultAny)
		{
			for (int i = 0; i < member.ElementInfo.Count; i++)
			{
				...
				string text2 = "(" + obTempVar2 + ".LocalName == " + GetLiteral(xmlTypeMapElementInfo.ElementName) + " && " + obTempVar2 + ".NamespaceURI == " + GetLiteral(xmlTypeMapElementInfo.Namespace) + ")";
		...
		if (_format == SerializationFormat.Literal)
		{
			WriteLine("WriteElementLiteral (" + obTempVar2 + ", \"\", \"\", false, true);");
		}
		else
		{
			WriteLine("WriteElementEncoded (" + obTempVar2 + ", \"\", \"\", false, true);");
		}
		if (!member.IsDefaultAny)
		{
			WriteLineUni("}");
			WriteLineInd("else");
			WriteLine("throw CreateUnknownAnyElementException (" + obTempVar2 + ".Name, " + obTempVar2 + ".NamespaceURI);");
			Unindent();
		}
		WriteLineUni("}");
		WriteLine("else " + obTempVar2 + ".WriteTo (Writer);");
		if (!flag)
		{
			WriteLineUni("}");
		}
	...
	private void GenerateGetArrayType(ListMap map, string itemCount, out string localName, out string ns)
	{
		...
		{
			GenerateGetArrayType((ListMap)xmlTypeMapElementInfo.MappedType.ObjectMap, "", out var localName2, out ns);
	...
	private void GenerateWriteInitCallbacks()
	{
		...
		foreach (XmlTypeMapping item2 in _mapsToGenerate)
		{
			if (item2 != null)
			{
				if (item2.TypeData.SchemaType == SchemaTypes.Enum)
				{
					WriteWriteEnumCallback(item2);
				}
				else
				{
					WriteWriteObjectCallback(item2);
				}
			}
		}
	}
	...
	private string GenerateReadMessage(XmlMembersMapping typeMap)
	{
		...
		if (typeMap.HasWrapperElement)
		{
			if (_format == SerializationFormat.Encoded)
			{
				...
				GenerateReadMembers(typeMap, (ClassMap)typeMap.ObjectMap, "parameters", isValueList: true, readBySoapOrder: false);
	...
	private void GenerateReadClassInstance(XmlTypeMapping typeMap, string isNullable, string checkType)
	{
		...
		WriteLine("");
		GenerateReadMembers(typeMap, (ClassMap)typeMap.ObjectMap, "ob", isValueList: false, readBySoapOrder: false);
		WriteLine("");
		GenerateEndHook();
		WriteLine("return ob;");
	}
	...
	private void GenerateReadMembers(XmlMapping xmlMap, ClassMap map, string ob, bool isValueList, bool readBySoapOrder)
	{
		...
		if (!GenerateReadHook(HookType.elements, type))
		{
			...
			if (map.ElementMembers != null && !readBySoapOrder)
			{
				...
				foreach (XmlTypeMapElementInfo allElementInfo in map.AllElementInfos)
				{
					if (allElementInfo.ExplicitOrder >= 0)
					{
						WriteLine("int idx = -1;");
						break;
					}
			...
			for (int j = 0; j < num2; j++)
			{
				...
				if (!readBySoapOrder)
				{
					...
					string text5 = (first ? "" : "else ");
					text5 += "if (";
					if (xmlTypeMapElementInfo.ExplicitOrder >= 0)
					{
						text5 = text5 + "idx < " + xmlTypeMapElementInfo.ExplicitOrder + "&& ";
					}
					...
					text5 = ((array[xmlTypeMapElementInfo.Member.Index] == null) ? (text5 + "true) {") : (text5 + "!" + array[xmlTypeMapElementInfo.Member.Index] + ") {"));
					WriteLineInd(text5);
				}
				...
				else
				{
					...
					if (xmlTypeMapElementInfo.ExplicitOrder >= 0)
					{
						WriteLine("idx = " + xmlTypeMapElementInfo.ExplicitOrder + ";");
					}
					if (_format == SerializationFormat.Encoded)
					{
			...
			if (!readBySoapOrder)
			{
				...
				if (map.XmlTextCollector != null)
				{
					...
					if (map.XmlTextCollector is XmlTypeMapMemberExpandable)
					{
						...
						TypeData typeData = ((!(xmlTypeMapMemberExpandable is XmlTypeMapMemberFlatList xmlTypeMapMemberFlatList2)) ? xmlTypeMapMemberExpandable.TypeData.ListItemTypeData : xmlTypeMapMemberFlatList2.ListMap.FindTextElement().TypeData);
						if (!GenerateReadArrayMemberHook(type, map.XmlTextCollector, array2[xmlTypeMapMemberExpandable.FlatArrayIndex]))
						{
							string value = ((typeData.Type == typeof(string)) ? "Reader.ReadString()" : GetReadXmlNode(typeData, wrapped: false));
					...
					else if (!GenerateReadMemberHook(type, map.XmlTextCollector))
					{
						...
						{
							GenerateSetMemberValue(xmlTypeMapMemberElement3, ob, strTempVar, isValueList);
						}
						else
						{
	...
	private void GenerateReadAttributeMembers(XmlMapping xmlMap, ClassMap map, string ob, bool isValueList, ref bool first)
	{
		...
		if (map.AttributeMembers != null)
		{
			...
			{
				WriteLineInd((first ? "" : "else ") + "if (Reader.LocalName == " + GetLiteral(attributeMember.AttributeName) + " && Reader.NamespaceURI == " + GetLiteral(attributeMember.Namespace) + ") {");
	...
	private bool IsReadOnly(XmlTypeMapping map, XmlTypeMapMember member, TypeData memType, bool isValueList)
	{
		...
		if (!member.IsReadOnly(map.TypeData.Type))
		{
			return !memType.HasPublicConstructor;
		}
		return true;
	...
	{
		GenerateSetMemberValue(member, ob, value, isValueList, initializingMember: false);
	}
	...
	private void GenerateSetMemberValue(XmlTypeMapMember member, string ob, string value, bool isValueList, bool initializingMember)
	{
		if (isValueList)
		{
			...
			if (member.IsOptionalValueType)
			{
				...
				WriteLine(ob + "[" + member.SpecifiedGlobalIndex + "] = " + text + ";");
			}
		}
		else
		{
			...
			if (member.IsOptionalValueType && member.IsValueSpecifiedSettable())
			{
				...
				WriteLine(ob + "." + member.Name + "Specified = " + text2 + ";");
			}
		}
	}
	...
	private void GenerateSetMemberValueFromAttr(XmlTypeMapMember member, string ob, string value, bool isValueList)
	{
		...
		GenerateSetMemberValue(member, ob, value, isValueList, initializingMember: true);
	}
	...
	private string GenerateReadListElement(XmlTypeMapping typeMap, string list, string isNullable, bool canCreateInstance)
	{
		...
		{
			WriteLineInd((flag2 ? "" : "else ") + "if (Reader.LocalName == " + GetLiteral(item.ElementName) + " && Reader.NamespaceURI == " + GetLiteral(item.Namespace) + ") {");
	...
	{
		EnumMap obj = (EnumMap)typeMap.ObjectMap;
		WriteLine("switch (" + val + ")");
		WriteLineInd("{");
		EnumMap.EnumMapMember[] members = obj.Members;
	...
	private void GenerateFixupCallbacks()
	{
		foreach (XmlMapping fixupCallback in _fixupCallbacks)
		{
			...
			string text = ((!flag) ? ((XmlTypeMapping)fixupCallback).TypeData.CSharpFullName : "object[]");
	...
	private string GetRootTypeName()
	{
		if (_typeMap is XmlTypeMapping)
		{
			return GetTypeFullName(((XmlTypeMapping)_typeMap).TypeData);
		}
		return "object[]";
	}
	...
	private string GetLiteral(object ob)
	{
		...
		if (ob is Enum)
		{
			...
			StringBuilder stringBuilder = new StringBuilder();
			string[] array = Enum.Format(ob.GetType(), ob, "g").Split(',');
