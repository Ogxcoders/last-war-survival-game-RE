...
public class XmlReflectionImporter
{
	...
	{
		return ImportMembersMapping(elementName, ns, members, hasWrapperElement, rpc, openModel: true);
	}
	...
	public XmlMembersMapping ImportMembersMapping(string elementName, string ns, XmlReflectionMember[] members, bool hasWrapperElement, bool rpc, bool openModel, XmlMappingAccess access)
	{
		...
		{
			XmlTypeMapMember xmlTypeMapMember = CreateMapMember(null, members[i], ns);
			xmlTypeMapMember.GlobalIndex = i;
			xmlTypeMapMember.CheckOptionalValueType(members);
			arrayList.Add(new XmlMemberMapping(members[i].MemberName, ns, xmlTypeMapMember, encodedFormat: false));
		}
		elementName = XmlConvert.EncodeLocalName(elementName);
		XmlMembersMapping result = new XmlMembersMapping(elementName, ns, hasWrapperElement, writeAccessors: false, (XmlMemberMapping[])arrayList.ToArray(typeof(XmlMemberMapping)))
		{
			RelatedMaps = relatedMaps,
			Format = SerializationFormat.Literal
		};
		if (includedTypes != null)
		{
			_ = (Type[])includedTypes.ToArray(typeof(Type));
		}
		return result;
	}
	...
	private XmlTypeMapping CreateTypeMapping(TypeData typeData, XmlRootAttribute root, string defaultXmlType, string defaultNamespace)
	{
		...
		XmlTypeMapping xmlTypeMapping = typeData.SchemaType switch
		{
			SchemaTypes.XmlSerializable => new XmlSerializableMapping(root, elementName, text, typeData, defaultXmlType, text2), 
			...
			_ => new XmlTypeMapping(elementName, text, typeData, defaultXmlType, flag ? text2 : null), 
		};
	...
	private XmlTypeMapping ImportClassMapping(TypeData typeData, XmlRootAttribute root, string defaultNamespace, bool isBaseType = false)
	{
		...
		List<XmlReflectionMember> reflectionMembers = GetReflectionMembers(type);
		bool? flag = null;
		foreach (XmlReflectionMember item in reflectionMembers)
		{
			int? order = item.XmlAttributes.Order;
			if (!flag.HasValue)
			{
				if (order.HasValue)
				{
			...
			else if (order.HasValue && flag != order.Value >= 0)
			{
		...
		foreach (XmlReflectionMember item2 in reflectionMembers)
		{
			string xmlTypeNamespace = registeredClrType.XmlTypeNamespace;
			if (item2.XmlAttributes.XmlIgnore)
			{
				continue;
			}
			if (item2.DeclaringType != null && item2.DeclaringType != type)
			{
				XmlTypeMapping xmlTypeMapping = ImportClassMapping(item2.DeclaringType, root, defaultNamespace, isBaseType: true);
				if (xmlTypeMapping.HasXmlTypeNamespace)
				{
			...
			try
			{
				XmlTypeMapMember xmlTypeMapMember = CreateMapMember(type, item2, xmlTypeNamespace);
				xmlTypeMapMember.CheckOptionalValueType(type);
				classMap.AddMember(xmlTypeMapMember);
			}
			catch (Exception innerException)
			{
				throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "There was an error reflecting field '{0}'.", item2.MemberName), innerException);
			}
		}
		...
		{
			XmlTypeMapping xmlTypeMapping2 = ImportClassMapping(type.BaseType, root, defaultNamespace, isBaseType: true);
	...
	private XmlTypeMapping ImportListMapping(TypeData typeData, XmlRootAttribute root, string defaultNamespace, XmlAttributes atts, int nestingLevel)
	{
		...
		foreach (XmlArrayItemAttribute xmlArrayItem in atts.XmlArrayItems)
		{
			...
			{
				Type type2 = ((xmlArrayItem.Type != null) ? xmlArrayItem.Type : listItemType);
				...
				xmlTypeMapElementInfo.Namespace = ((xmlArrayItem.Namespace != null) ? xmlArrayItem.Namespace : defaultNamespace);
				...
				xmlTypeMapElementInfo.IsNullable = (!xmlArrayItem.IsNullableSpecified || xmlArrayItem.IsNullable) && CanBeNull(xmlTypeMapElementInfo.TypeData);
	...
	private XmlTypeMapMember CreateMapMember(Type declaringType, XmlReflectionMember rmember, string defaultNamespace)
	{
		...
		else if (typeData.SchemaType == SchemaTypes.Array)
		{
			...
			{
				XmlTypeMapMemberList obj = new XmlTypeMapMemberList
				{
					ElementInfo = new XmlTypeMapElementInfoList()
				};
				XmlTypeMapElementInfo xmlTypeMapElementInfo = new XmlTypeMapElementInfo(obj, typeData);
				...
				obj.ElementInfo.Add(xmlTypeMapElementInfo);
				xmlTypeMapMember = obj;
			}
		}
		else
		{
	...
	private XmlTypeMapElementInfoList ImportElementInfo(Type cls, string defaultName, string defaultNamespace, Type defaultType, XmlTypeMapMemberElement member, XmlAttributes atts)
	{
		...
		{
			Type type2 = ((xmlElement.Type != null) ? xmlElement.Type : defaultType);
			...
			{
				xmlTypeMapElementInfo2.Namespace = ((xmlElement.Namespace != null) ? xmlElement.Namespace : defaultNamespace);
			}
			if (xmlElement.IsNullable && !xmlTypeMapElementInfo2.IsNullable)
			{
				xmlTypeMapElementInfo2.IsNullable = xmlElement.IsNullable;
			}
			xmlTypeMapElementInfo2.ExplicitOrder = xmlElement.Order;
			...
			if (enumMap != null)
			{
				...
				xmlTypeMapElementInfo2.ChoiceValue = Enum.Parse(type, enumName, ignoreCase: false);
			}
			xmlTypeMapElementInfoList.Add(xmlTypeMapElementInfo2);
		}
	...
	private XmlTypeMapElementInfoList ImportAnyElementInfo(string defaultNamespace, XmlReflectionMember rmember, XmlTypeMapMemberElement member, XmlAttributes atts)
	{
		...
		foreach (XmlAnyElementAttribute xmlAnyElement in atts.XmlAnyElements)
		{
			...
			if (xmlAnyElement.Name.Length != 0)
			{
				...
				xmlTypeMapElementInfo.Namespace = ((xmlAnyElement.Namespace != null) ? xmlAnyElement.Namespace : "");
			}
			else
			{
			...
			xmlTypeMapElementInfo.ExplicitOrder = xmlAnyElement.Order;
			xmlTypeMapElementInfoList.Add(xmlTypeMapElementInfo);
		}
		return xmlTypeMapElementInfoList;
	}
	...
	public void IncludeTypes(ICustomAttributeProvider provider)
	{
		...
		{
			XmlIncludeAttribute xmlIncludeAttribute = (XmlIncludeAttribute)customAttributes[i];
