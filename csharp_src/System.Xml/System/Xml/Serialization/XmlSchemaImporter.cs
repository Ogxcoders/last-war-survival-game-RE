...
public class XmlSchemaImporter : SchemaImporter
{
	...
	private ArrayList fixup_registered_types = new ArrayList();
	...
	public XmlMembersMapping ImportAnyType(XmlQualifiedName typeName, string elementName)
	{
		if (typeName == XmlQualifiedName.Empty)
		{
			...
			xmlTypeMapMemberAnyElement.ElementInfo.Add(CreateElementInfo(typeName.Namespace, xmlTypeMapMemberAnyElement, typeName.Name, xmlTypeMapMemberAnyElement.TypeData, isNillable: true, XmlSchemaForm.None, -1));
		...
		{
			throw new InvalidOperationException("Referenced type '" + typeName?.ToString() + "' not found");
		}
		if (!CanBeAnyElement(xmlSchemaComplexType))
		{
			throw new InvalidOperationException("The type '" + typeName?.ToString() + "' is not valid for a collection of any elements");
		}
	...
	public XmlTypeMapping ImportDerivedTypeMapping(XmlQualifiedName name, Type baseType, bool baseTypeCanBeIndirect)
	{
		...
		if (encodedFormat)
		{
			...
			if (stype == null)
			{
				throw new InvalidOperationException("Schema type '" + name?.ToString() + "' not found or not valid");
			}
		}
	...
	public XmlMembersMapping ImportMembersMapping(XmlQualifiedName name)
	{
		...
		{
			throw new InvalidOperationException("Schema element '" + name?.ToString() + "' not found or not valid");
		}
		...
		else
		{
			...
			if (obj == null)
			{
				...
				throw new InvalidOperationException("Schema type '" + xmlSchemaElement.SchemaTypeName?.ToString() + "' not found");
			}
	...
	public XmlMembersMapping ImportMembersMapping(XmlQualifiedName[] names)
	{
		...
		for (int i = 0; i < names.Length; i++)
		{
			...
			if (xmlSchemaElement == null)
			{
				throw new InvalidOperationException("Schema element '" + names[i]?.ToString() + "' not found");
			}
			...
			array[i] = ImportMemberMapping(xmlSchemaElement.Name, xmlQualifiedName.Namespace, xmlSchemaElement.IsNillable, elementTypeData, map, i);
		}
		BuildPendingMaps();
	...
	internal XmlMembersMapping ImportEncodedMembersMapping(string name, string ns, SoapSchemaMember[] members, bool hasWrapperElement)
	{
		...
		for (int i = 0; i < members.Length; i++)
		{
			...
			array[i] = ImportMemberMapping(members[i].MemberName, members[i].MemberType.Namespace, isNullable: true, typeData, typeMapping, i);
		}
		BuildPendingMaps();
	...
	internal XmlMembersMapping ImportEncodedMembersMapping(string name, string ns, SoapSchemaMember member)
	{
		...
		{
			throw new InvalidOperationException("Schema type '" + member.MemberType?.ToString() + "' not found or not valid");
		}
	...
	private XmlMemberMapping[] ImportMembersMappingComposite(XmlSchemaComplexType stype, XmlQualifiedName refer)
	{
		...
		{
			throw new InvalidOperationException("Schema element '" + refer?.ToString() + "' cannot be imported as XmlMembersMapping");
		}
		CodeIdentifiers classIds = new CodeIdentifiers();
	...
	private XmlMemberMapping ImportMemberMapping(string name, string ns, bool isNullable, TypeData type, XmlTypeMapping emap, int order)
	{
		...
		xmlTypeMapMemberElement.ElementInfo.Add(CreateElementInfo(ns, xmlTypeMapMemberElement, name, type, isNullable, XmlSchemaForm.None, emap, order));
	...
	public XmlTypeMapping ImportTypeMapping(XmlQualifiedName name)
	{
		...
		{
			schemas.Compile(null, fullCompile: true);
		}
		XmlSchemaElement xmlSchemaElement = (XmlSchemaElement)schemas.Find(name, typeof(XmlSchemaElement));
	...
	private bool LocateElement(XmlSchemaElement elem, out XmlQualifiedName qname, out XmlSchemaType stype)
	{
		...
		else
		{
			...
			if (obj == null)
			{
				...
				throw new InvalidOperationException("Schema type '" + elem.SchemaTypeName?.ToString() + "' not found");
			}
	...
	private XmlTypeMapping ImportType(XmlQualifiedName name, XmlQualifiedName root, bool throwOnError)
	{
		...
		if (xmlSchemaType == null)
		{
			if (throwOnError)
			{
				...
				{
					throw new InvalidOperationException("Referenced type '" + name?.ToString() + "' valid only for encoded SOAP.");
				}
				throw new InvalidOperationException("Referenced type '" + name?.ToString() + "' not found.");
			}
			return null;
		}
	...
	private void RegisterMapFixup(XmlTypeMapping map, XmlQualifiedName typeQName, XmlSchemaComplexType stype)
	{
		...
		{
			throw new InvalidOperationException($"Circular dependency for schema type {map.ElementName} in namespace {map.Namespace}");
		}
	...
	private IEnumerable<XmlSchemaAttribute> EnumerateAttributes(XmlSchemaObjectCollection col, List<XmlSchemaAttributeGroup> recurse)
	{
		...
		{
			if (item is XmlSchemaAttributeGroupRef)
			{
				XmlSchemaAttributeGroupRef xmlSchemaAttributeGroupRef = (XmlSchemaAttributeGroupRef)item;
				XmlSchemaAttributeGroup xmlSchemaAttributeGroup = FindRefAttributeGroup(xmlSchemaAttributeGroupRef.RefName);
				...
				{
					throw new InvalidOperationException($"Referenced AttributeGroup '{xmlSchemaAttributeGroupRef.RefName}' was not found.");
				}
				foreach (XmlSchemaAttribute item2 in EnumerateAttributes(xmlSchemaAttributeGroup.Attributes, recurse))
				{
					yield return item2;
				}
			}
			...
			{
				yield return (XmlSchemaAttribute)item;
			}
		}
	}
	...
	private XmlSchemaObjectCollection CollectAttributeUsesNonOverlap(XmlSchemaObjectCollection src, ClassMap map)
	{
		...
		foreach (XmlSchemaAttribute item in EnumerateAttributes(src, new List<XmlSchemaAttributeGroup>()))
		{
	...
	private ListMap BuildEncodedArrayMap(string type, string ns, out TypeData arrayTypeData)
	{
		...
		if (text.IndexOf("[") != -1)
		{
			...
			CreateArrayTypeMapping(typeQName, arrayTypeData2).ObjectMap = objectMap;
		}
		else
		{
		...
		listMap.ItemInfo.Add(CreateElementInfo("", null, "Item", arrayTypeData2, isNillable: true, XmlSchemaForm.None, -1));
		return listMap;
	}
		...
		foreach (XmlSchemaObject item in items)
		{
			if (item is XmlSchemaElement)
			{
				...
				if (xmlSchemaElement.MaxOccurs == 1m && !multiValue)
				{
					...
					xmlTypeMapMemberElement.ElementInfo.Add(CreateElementInfo(ns, xmlTypeMapMemberElement, refElement.Name, typeData, refElement.IsNillable, refElement.Form, map, items.IndexOf(item)));
				...
				else
				{
					...
					xmlTypeMapMemberFlatList.ElementInfo.Add(CreateElementInfo(ns, xmlTypeMapMemberFlatList, refElement.Name, typeData, refElement.IsNillable, refElement.Form, map, items.IndexOf(item)));
					xmlTypeMapMemberFlatList.ListMap.ItemInfo = xmlTypeMapMemberFlatList.ElementInfo;
					cmap.AddMember(xmlTypeMapMemberFlatList);
				}
			}
			else if (item is XmlSchemaAny)
			{
				...
				Type type = ((!(xmlSchemaAny.MaxOccurs != 1m || multiValue)) ? (isMixed ? typeof(XmlNode) : typeof(XmlElement)) : (isMixed ? typeof(XmlNode[]) : typeof(XmlElement[])));
	...
	private object ImportDefaultValue(TypeData typeData, string value)
	{
		...
		{
			XmlTypeMapping typeMapping = GetTypeMapping(typeData);
			return ((EnumMap)typeMapping.ObjectMap).GetEnumName(typeMapping.TypeFullName, value) ?? throw new InvalidOperationException("'" + value + "' is not a valid enumeration value");
		}
	...
	private void ImportChoiceContent(XmlQualifiedName typeQName, ClassMap cmap, XmlSchemaChoice choice, CodeIdentifiers classIds, bool multiValue)
	{
		...
		for (int num = xmlTypeMapElementInfoList.Count - 1; num >= 0; num--)
		{
			...
			if (cmap.GetElement(xmlTypeMapElementInfo.ElementName, xmlTypeMapElementInfo.Namespace, xmlTypeMapElementInfo.ExplicitOrder) != null || xmlTypeMapElementInfoList.IndexOfElement(xmlTypeMapElementInfo.ElementName, xmlTypeMapElementInfo.Namespace) != num)
			{
		...
		if (flag)
		{
			...
			xmlTypeMapMemberElement2.ElementInfo.Add(CreateElementInfo(typeQName.Namespace, xmlTypeMapMemberElement2, xmlTypeMapMemberElement2.Name, xmlTypeMapMemberElement2.TypeData, isNillable: false, XmlSchemaForm.None, -1));
	...
	private bool ImportChoices(XmlQualifiedName typeQName, XmlTypeMapMember member, XmlTypeMapElementInfoList choices, XmlSchemaObjectCollection items)
	{
		...
		foreach (XmlSchemaObject item in items)
		{
			...
			if (xmlSchemaObject is XmlSchemaElement)
			{
				...
				string ns;
				XmlSchemaElement refElement = GetRefElement(typeQName, xmlSchemaElement, out ns);
				choices.Add(CreateElementInfo(ns, member, refElement.Name, elementTypeData, refElement.IsNillable, refElement.Form, map, -1));
	...
	private void ImportSimpleContent(XmlQualifiedName typeQName, XmlTypeMapping map, XmlSchemaSimpleContent content, CodeIdentifiers classIds, bool isMixed)
	{
		...
		if (!IsPrimitiveTypeNamespace(contentBaseType.Namespace))
		{
			...
			{
				foreach (XmlTypeMapMember allMember in ((ClassMap)xmlTypeMapping.ObjectMap).AllMembers)
				{
	...
	private TypeData FindBuiltInType(XmlQualifiedName qname)
	{
		...
		throw new InvalidOperationException("Definition of type '" + qname?.ToString() + "' not found");
	}
	...
	private XmlTypeMapping ImportClassSimpleType(XmlQualifiedName typeQName, XmlSchemaSimpleType stype, XmlQualifiedName root)
	{
		...
		if (stype.Content is XmlSchemaSimpleTypeList)
		{
			...
			ListMap listMap = new ListMap();
			listMap.ItemInfo = new XmlTypeMapElementInfoList();
			listMap.ItemInfo.Add(CreateElementInfo(typeQName.Namespace, null, "Item", typeData.ListItemTypeData, isNillable: false, XmlSchemaForm.None, -1));
	...
	private bool CanBeEnum(XmlSchemaSimpleType stype)
	{
		...
		if (stype.Content is XmlSchemaSimpleTypeList)
		{
			...
			{
				return CanBeEnum(xmlSchemaSimpleTypeList.ItemType);
			}
			return false;
		}
		return false;
	...
	private bool CanBeArray(XmlQualifiedName typeQName, XmlSchemaComplexType stype)
	{
		...
		if (!stype.IsMixed)
		{
			return CanBeArray(typeQName, stype.Particle, multiValue: false);
		}
		return false;
	}
	...
	private bool CanBeArray(XmlQualifiedName typeQName, XmlSchemaParticle particle, bool multiValue)
	{
		...
		if (particle is XmlSchemaElement)
		{
			...
			if (multiValue)
			{
				return !typeQName.Equals(((XmlSchemaElement)particle).SchemaTypeName);
			}
			return false;
		}
	...
	private bool CheckChoiceType(XmlQualifiedName typeQName, XmlSchemaParticle particle, ArrayList types, ref bool multiValue)
	{
		...
		else
		{
			if (particle is XmlSchemaSequence)
			{
				foreach (XmlSchemaParticle item in (particle as XmlSchemaSequence).Items)
				{
	...
	{
		if (stype.Particle is XmlSchemaSequence xmlSchemaSequence && xmlSchemaSequence.Items.Count == 1)
		{
			return xmlSchemaSequence.Items[0] is XmlSchemaAny;
		}
		return false;
	}
	...
	private XmlTypeMapping ImportXmlSerializableMapping(string ns)
	{
		...
		registeredTypeMapping = new XmlTypeMapping("System.Data.DataSet", "", typeData, "System.Data.DataSet", ns);
	...
	private XmlTypeMapElementInfo CreateElementInfo(string ns, XmlTypeMapMember member, string name, TypeData typeData, bool isNillable, XmlSchemaForm form, XmlTypeMapping emap, int order)
	{
		...
		xmlTypeMapElementInfo.Form = GetForm(form, ns, forElement: true);
		if (typeData.IsComplexType)
		{
	...
	private XmlSchemaAttribute GetRefAttribute(XmlQualifiedName typeQName, XmlSchemaAttribute attr, out string ns)
	{
		if (!attr.RefName.IsEmpty)
		{
			...
			return FindRefAttribute(attr.RefName) ?? throw new InvalidOperationException("The attribute " + attr.RefName?.ToString() + " is missing");
		}
	...
	private TypeData GetElementTypeData(XmlQualifiedName typeQName, XmlSchemaElement elem, XmlQualifiedName root, out XmlTypeMapping map)
	{
		...
		if (!elem.RefName.IsEmpty)
		{
			XmlSchemaElement obj = FindRefElement(elem) ?? throw new InvalidOperationException("Global element not found: " + elem.RefName);
			root = elem.RefName;
			elem = obj;
			sharedAnnType = true;
		}
	...
	private TypeData GetAttributeTypeData(XmlQualifiedName typeQName, XmlSchemaAttribute attr)
	{
		...
		if (!attr.RefName.IsEmpty)
		{
			attr = FindRefAttribute(attr.RefName) ?? throw new InvalidOperationException("Global attribute not found: " + attr.RefName);
			sharedAnnType = true;
		}
	...
	private XmlTypeMapping GetTypeMapping(TypeData typeData)
	{
		...
		if (typeData.IsListType)
		{
			...
			ListMap listMap = new ListMap();
			listMap.ItemInfo = new XmlTypeMapElementInfoList();
			listMap.ItemInfo.Add(CreateElementInfo(typeMapping.Namespace, null, typeData.ListItemTypeData.XmlType, typeData.ListItemTypeData, isNillable: false, XmlSchemaForm.None, -1));
	...
	{
		return ((XmlSchemaGroup)schemas.Find(refGroup.RefName, typeof(XmlSchemaGroup))).Particle;
	}
	...
	private string GetDocumentation(XmlSchemaAnnotated elem)
	{
		...
		{
			if (item is XmlSchemaDocumentation { Markup: not null } xmlSchemaDocumentation && xmlSchemaDocumentation.Markup.Length != 0)
			{
	...
	private bool IsPrimitiveTypeNamespace(string ns)
	{
		if (!(ns == "http://www.w3.org/2001/XMLSchema"))
		{
			if (encodedFormat)
			{
				return ns == "http://schemas.xmlsoap.org/soap/encoding/";
			}
			return false;
		}
		return true;
