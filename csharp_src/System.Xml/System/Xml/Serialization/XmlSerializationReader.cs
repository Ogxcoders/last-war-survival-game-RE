...
public abstract class XmlSerializationReader : XmlSerializationGeneratedCode
{
	...
	{
		return new InvalidOperationException($"The type '{typeName}' cannot be serialized because its parameterless constructor is decorated with declarative security permission attributes. Consider using imperative asserts or demands in the constructor.");
	}

	protected Exception CreateInaccessibleConstructorException(string typeName)
	{
		return new InvalidOperationException($"{typeName} cannot be serialized because it does not have a default public constructor.");
	}

	protected Exception CreateAbstractTypeException(string name, string ns)
	{
		return new InvalidOperationException("The specified type is abstrace: name='" + name + "' namespace='" + ns + "', at " + CurrentTag());
	}
	...
	{
		return new InvalidOperationException($"Could not serialize {name}. Default constructors are required for collections and enumerators.");
	}

	protected Exception CreateUnknownConstantException(string value, Type enumType)
	{
		return new InvalidOperationException($"'{value}' is not a valid value for {enumType}.");
	}

	protected Exception CreateUnknownNodeException()
	{
		return new InvalidOperationException(CurrentTag() + " was not expected");
	}

	protected Exception CreateUnknownTypeException(XmlQualifiedName type)
	{
		return new InvalidOperationException("The specified type was not recognized: name='" + type.Name + "' namespace='" + type.Namespace + "', at " + CurrentTag());
	}
	...
	protected XmlQualifiedName GetXsiType()
	{
		...
		return new XmlQualifiedName(attribute.Substring(num + 1), Reader.LookupNamespace(prefix));
	}
	...
	protected object ReadReferencedElement(string name, string ns)
	{
		...
		{
			CollectionFixup collectionFixup = ((collFixups != null) ? ((CollectionFixup)collFixups[attribute]) : null);
	...
	protected void ReadReferencedElements()
	{
		...
		if (collFixups != null)
		{
			foreach (CollectionFixup value in collFixups.Values)
			{
				value.Callback(value.Collection, value.CollectionItems);
			}
		}
		if (fixups != null)
		{
	...
	protected IXmlSerializable ReadSerializable(IXmlSerializable serializable, bool wrappedAny)
	{
		...
		if (wrappedAny)
		{
			...
			if (reader.NodeType == XmlNodeType.EndElement && reader.LocalName == text && reader.NamespaceURI == text2)
			{
				reader.Read();
			}
		}
	...
	{
		return ReadTypedPrimitive(type, reportUnknown: false);
	}
	...
	protected XmlDocument ReadXmlDocument(bool wrapped)
	{
		...
		reader.MoveToContent();
		XmlDocument xmlDocument = new XmlDocument(reader.NameTable);
	...
	protected XmlQualifiedName ToXmlQualifiedName(string value)
	{
		...
		if (num < 0)
		{
			name = reader.NameTable.Add(array);
		...
		else
		{
			...
			name = reader.NameTable.Add(value.Substring(num + 1));
		}
	...
	private void OnUnknownNode(XmlNode node, object o, string qnames)
	{
		...
		if (eventSource != null)
		{
			eventSource.OnUnknownNode(new XmlNodeEventArgs(node, lineNumber, linePosition, o));
		}
