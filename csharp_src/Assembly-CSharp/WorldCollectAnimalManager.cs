...
public class WorldCollectAnimalManager : WorldManagerBase
{
	private Dictionary<long, CollectAnimalModel.Param> _animDict = new Dictionary<long, CollectAnimalModel.Param>();

	public WorldCollectAnimalManager(WorldScene scene)
		: base(scene)
	{
	}
	...
	public void CreateAnimalObject(long uuid)
	{
		...
		CollectPointInfo collectRangePoint = world.GetCollectRangePoint(buildingDataByUuid.pointId);
		...
		Log.Info("CollectAnimalModel : WorldCollectAnimalManager : uuid : {0}", uuid);
