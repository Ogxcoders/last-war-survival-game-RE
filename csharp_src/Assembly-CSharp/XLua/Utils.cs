...
public static class Utils
{
	...
	private static lua_CSFunction genFieldGetter(Type type, FieldInfo field)
	{
		...
		return delegate(IntPtr L)
		{
			...
			{
				return Lua.luaL_error(L, string.Concat("Expected type ", type, ", but got ", (obj == null) ? "null" : obj.GetType().ToString(), ", while get field ", field));
			}
	...
	private static lua_CSFunction genFieldSetter(Type type, FieldInfo field)
	{
		if (field.IsStatic)
		{
			return delegate(IntPtr L)
			{
				object obj = ObjectTranslatorPool.Instance.Find(L).GetObject(L, 1, field.FieldType);
				if (field.FieldType.IsValueType() && Nullable.GetUnderlyingType(field.FieldType) == null && obj == null)
				{
		...
		return delegate(IntPtr L)
		{
			...
			if (obj == null || !type.IsInstanceOfType(obj))
			{
				return Lua.luaL_error(L, string.Concat("Expected type ", type, ", but got ", (obj == null) ? "null" : obj.GetType().ToString(), ", while set field ", field));
			}
			object obj2 = objectTranslator.GetObject(L, 2, field.FieldType);
			if (field.FieldType.IsValueType() && Nullable.GetUnderlyingType(field.FieldType) == null && obj2 == null)
			{
				return Lua.luaL_error(L, type.Name + "." + field.Name + " Expected type " + field.FieldType);
			}
	...
	private static lua_CSFunction genItemGetter(Type type, PropertyInfo[] props)
	{
		...
		{
			params_type[num] = props[num].GetIndexParameters()[0].ParameterType;
		}
		...
		return delegate(IntPtr L)
		{
			...
			if (obj == null || !type.IsInstanceOfType(obj))
			{
				return Lua.luaL_error(L, string.Concat("Expected type ", type, ", but got ", (obj == null) ? "null" : obj.GetType().ToString(), ", while get prop ", props[0].Name));
			}
			for (int i = 0; i < props.Length; i++)
			{
				if (objectTranslator.Assignable(L, 2, params_type[i]))
				{
					PropertyInfo propertyInfo = props[i];
					try
					{
						object obj2 = objectTranslator.GetObject(L, 2, params_type[i]);
						arg[0] = obj2;
					...
					{
						return Lua.luaL_error(L, string.Concat("try to get ", type, ".", propertyInfo.Name, " throw a exception:", ex, ",stack:", ex.StackTrace));
					}
				}
			}
	...
	private static lua_CSFunction genItemSetter(Type type, PropertyInfo[] props)
	{
		...
		return delegate(IntPtr L)
		{
			...
			if (obj == null || !type.IsInstanceOfType(obj))
			{
				return Lua.luaL_error(L, string.Concat("Expected type ", type, ", but got ", (obj == null) ? "null" : obj.GetType().ToString(), ", while set prop ", props[0].Name));
			}
			for (int i = 0; i < props.Length; i++)
			{
				if (objectTranslator.Assignable(L, 2, params_type[i]))
				{
					...
					try
					{
						arg[0] = objectTranslator.GetObject(L, 2, params_type[i]);
						object obj2 = objectTranslator.GetObject(L, 3, propertyInfo.PropertyType);
						if (obj2 == null)
						{
							return Lua.luaL_error(L, type.Name + "." + propertyInfo.Name + " Expected type " + propertyInfo.PropertyType);
						}
						propertyInfo.SetValue(obj, obj2, arg);
						Lua.lua_pushboolean(L, value: true);
						return 1;
					}
					catch (Exception ex)
					{
						return Lua.luaL_error(L, string.Concat("try to set ", type, ".", propertyInfo.Name, " throw a exception:", ex, ",stack:", ex.StackTrace));
					}
				}
			}
			Lua.lua_pushboolean(L, value: false);
	...
	private static lua_CSFunction genEnumCastFrom(Type type)
	{
		return delegate(IntPtr L)
		{
			...
			{
				return Lua.luaL_error(L, string.Concat("cast to ", type, " exception:", ex));
			}
		};
	}
	...
	internal static int LazyReflectionCall(IntPtr L)
	{
		try
		{
			...
			{
				EventInfo eventInfo = v.GetEvent(memberName);
				if (eventInfo == null)
				{
					return Lua.luaL_error(L, "can not find " + memberName + " for " + v);
				}
				...
				lua_CSFunction = objectTranslator.methodWrapsCache.GetEventWrap(v, eventInfo.Name);
				break;
			}
			default:
	...
	public static void SetCSTable(IntPtr L, Type type, int cls_table)
	{
		...
		for (int i = 0; i < pathOfType.Count - 1; i++)
		{
			...
			if (Lua.xlua_pgettable(L, -2) != 0)
			{
				...
				throw new Exception(string.Concat("SetCSTable for [", type, "] error: ", text));
			}
			if (Lua.lua_isnil(L, -1))
			{
				Lua.lua_pop(L, 1);
			...
			{
				Lua.lua_settop(L, num);
				throw new Exception(string.Concat("SetCSTable for [", type, "] error: ancestors is not a table!"));
			}
			Lua.lua_remove(L, -2);
		}
