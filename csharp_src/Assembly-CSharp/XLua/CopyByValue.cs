...
public static class CopyByValue
{
	...
	public static void UnPack(ObjectTranslator translator, IntPtr L, int idx, out TouchInfo val)
	{
		val = default(TouchInfo);
		int num = Lua.lua_gettop(L);
		if (Utils.LoadField(L, idx, "pointerId"))
		{
			translator.Get(L, num + 1, out val.pointerId);
		}
		Lua.lua_pop(L, 1);
		if (Utils.LoadField(L, idx, "pointerPos"))
		{
			translator.Get(L, num + 1, out val.pointerPos);
		}
		Lua.lua_pop(L, 1);
		if (Utils.LoadField(L, idx, "deltaPos"))
		{
			translator.Get(L, num + 1, out val.deltaPos);
		}
		Lua.lua_pop(L, 1);
	}

	public static bool Pack(IntPtr buff, int offset, TouchInfo field)
	{
		if (!Pack(buff, offset, field.pointerId))
		{
			return false;
		}
		if (!Pack(buff, offset + 4, field.pointerPos))
		{
			return false;
		}
		if (!Pack(buff, offset + 12, field.deltaPos))
		{
			return false;
		}
		return true;
	...
	public static bool UnPack(IntPtr buff, int offset, out TouchInfo field)
	{
		field = default(TouchInfo);
		if (!UnPack(buff, offset, out field.pointerId))
		{
			return false;
		}
		if (!UnPack(buff, offset + 4, out field.pointerPos))
		{
			return false;
		}
		if (!UnPack(buff, offset + 12, out field.deltaPos))
		{
			return false;
		}
		return true;
