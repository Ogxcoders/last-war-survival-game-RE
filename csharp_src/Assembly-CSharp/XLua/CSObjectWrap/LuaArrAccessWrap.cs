...
public class LuaArrAccessWrap
{
	public static void __Register(IntPtr L)
	{
		...
		Type typeFromHandle = typeof(LuaArrAccess);
	...
	private static int __CreateInstance(IntPtr L)
	{
		try
		{
			...
			{
				LuaArrAccess o = new LuaArrAccess();
		...
		return Lua.luaL_error(L, "invalid arguments to LuaArrAccess constructor!");
	}
	...
	private static int _m_ToString(IntPtr L)
	{
		try
		{
			string str = ((LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).ToString();
	...
	private static int _m_OnPin(IntPtr L)
	{
		try
		{
			LuaArrAccess obj = (LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
	...
	private static int _m_OnGC(IntPtr L)
	{
		try
		{
			((LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).OnGC();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_AutoDetectArch(IntPtr L)
	{
		try
		{
			((LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).AutoDetectArch();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_IsValid(IntPtr L)
	{
		try
		{
			bool value = ((LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).IsValid();
	...
	private static int _m_GetArrayCapacity(IntPtr L)
	{
		try
		{
			uint arrayCapacity = ((LuaArrAccess)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).GetArrayCapacity();
