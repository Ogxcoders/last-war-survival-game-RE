...
public class UnityEngineUIDropdownOptionDataWrap
{
	public static void __Register(IntPtr L)
	{
		...
		Type typeFromHandle = typeof(Dropdown.OptionData);
	...
	private static int __CreateInstance(IntPtr L)
	{
		try
		{
			...
			{
				Dropdown.OptionData o2 = new Dropdown.OptionData(Lua.lua_tostring(L, 2));
				objectTranslator.Push(L, o2);
				return 1;
			}
			if (Lua.lua_gettop(L) == 2 && objectTranslator.Assignable<Sprite>(L, 2))
			{
				Dropdown.OptionData o3 = new Dropdown.OptionData((Sprite)objectTranslator.GetObject(L, 2, typeof(Sprite)));
		...
		return Lua.luaL_error(L, "invalid arguments to UnityEngine.UI.Dropdown.OptionData constructor!");
	}
	...
	private static int _g_get_text(IntPtr L)
	{
		try
		{
			Dropdown.OptionData optionData = (Dropdown.OptionData)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
	...
	private static int _g_get_image(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			Dropdown.OptionData optionData = (Dropdown.OptionData)objectTranslator.FastGetCSObj(L, 1);
	...
	private static int _s_set_text(IntPtr L)
	{
		try
		{
			((Dropdown.OptionData)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).text = Lua.lua_tostring(L, 2);
		}
		catch (Exception ex)
		{
			return Lua.luaL_error(L, "c# exception:" + ex);
	...
	private static int _s_set_image(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			((Dropdown.OptionData)objectTranslator.FastGetCSObj(L, 1)).image = (Sprite)objectTranslator.GetObject(L, 2, typeof(Sprite));
		}
		catch (Exception ex)
		{
			return Lua.luaL_error(L, "c# exception:" + ex);
