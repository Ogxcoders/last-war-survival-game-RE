...
public class SystemCollectionsGenericDictionary_2_ResourceTypeSystemInt64_Wrap
{
	public static void __Register(IntPtr L)
	{
		...
		Type typeFromHandle = typeof(Dictionary<ResourceType, long>);
	...
	private static int __CreateInstance(IntPtr L)
	{
		try
		{
			...
			{
				Dictionary<ResourceType, long> o = new Dictionary<ResourceType, long>();
				objectTranslator.Push(L, o);
				return 1;
			}
			if (Lua.lua_gettop(L) == 2 && LuaTypes.LUA_TNUMBER == Lua.lua_type(L, 2))
			{
				Dictionary<ResourceType, long> o2 = new Dictionary<ResourceType, long>(Lua.xlua_tointeger(L, 2));
				objectTranslator.Push(L, o2);
				return 1;
			}
			if (Lua.lua_gettop(L) == 2 && objectTranslator.Assignable<IEqualityComparer<ResourceType>>(L, 2))
			{
				Dictionary<ResourceType, long> o3 = new Dictionary<ResourceType, long>((IEqualityComparer<ResourceType>)objectTranslator.GetObject(L, 2, typeof(IEqualityComparer<ResourceType>)));
			...
			if (Lua.lua_gettop(L) == 3 && LuaTypes.LUA_TNUMBER == Lua.lua_type(L, 2) && objectTranslator.Assignable<IEqualityComparer<ResourceType>>(L, 3))
			{
				...
				Dictionary<ResourceType, long> o4 = new Dictionary<ResourceType, long>(capacity, comparer);
		...
		return Lua.luaL_error(L, "invalid arguments to System.Collections.Generic.Dictionary<ResourceType, long> constructor!");
	}
	...
	public static int __CSIndexer(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			if (objectTranslator.Assignable<Dictionary<ResourceType, long>>(L, 1) && objectTranslator.Assignable<ResourceType>(L, 2))
			{
				Dictionary<ResourceType, long> dictionary = (Dictionary<ResourceType, long>)objectTranslator.FastGetCSObj(L, 1);
				objectTranslator.Get(L, 2, out ResourceType val);
				Lua.lua_pushboolean(L, value: true);
				Lua.lua_pushint64(L, dictionary[val]);
				return 2;
			}
		}
		catch (Exception ex)
	...
	public static int __NewIndexer(IntPtr L)
	{
		...
		try
		{
			if (objectTranslator.Assignable<Dictionary<ResourceType, long>>(L, 1) && objectTranslator.Assignable<ResourceType>(L, 2) && (LuaTypes.LUA_TNUMBER == Lua.lua_type(L, 3) || Lua.lua_isint64(L, 3)))
			{
				Dictionary<ResourceType, long> obj = (Dictionary<ResourceType, long>)objectTranslator.FastGetCSObj(L, 1);
				objectTranslator.Get(L, 2, out ResourceType val);
				obj[val] = Lua.lua_toint64(L, 3);
				Lua.lua_pushboolean(L, value: true);
				return 1;
			}
		}
	...
	private static int _m_Clear(IntPtr L)
	{
		try
		{
			((Dictionary<ResourceType, long>)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).Clear();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_ContainsValue(IntPtr L)
	{
		try
		{
			Dictionary<ResourceType, long> obj = (Dictionary<ResourceType, long>)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
			long value = Lua.lua_toint64(L, 2);
	...
	private static int _m_GetEnumerator(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			Dictionary<ResourceType, long>.Enumerator enumerator = ((Dictionary<ResourceType, long>)objectTranslator.FastGetCSObj(L, 1)).GetEnumerator();
	...
	private static int _g_get_Count(IntPtr L)
	{
		try
		{
			Dictionary<ResourceType, long> dictionary = (Dictionary<ResourceType, long>)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
