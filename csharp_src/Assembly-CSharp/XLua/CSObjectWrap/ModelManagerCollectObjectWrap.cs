...
public class ModelManagerCollectObjectWrap
{
	public static void __Register(IntPtr L)
	{
		ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		Type typeFromHandle = typeof(ModelManager.CollectObject);
	...
	private static int __CreateInstance(IntPtr L)
	{
		try
		{
			...
			if (Lua.lua_gettop(L) == 5 && objectTranslator.Assignable<ModelManager>(L, 2) && LuaTypes.LUA_TNUMBER == Lua.lua_type(L, 3) && objectTranslator.Assignable<ModelManager.ModelObjectType>(L, 4) && (Lua.lua_isnil(L, 5) || Lua.lua_type(L, 5) == LuaTypes.LUA_TTABLE))
			{
				...
				ModelManager.CollectObject o = new ModelManager.CollectObject(param: (LuaTable)objectTranslator.GetObject(L, 5, typeof(LuaTable)), parent: parent, pointId: pointId, modelObjectType: val);
		...
		return Lua.luaL_error(L, "invalid arguments to ModelManager.CollectObject constructor!");
	}
	...
	private static int _m_Destroy(IntPtr L)
	{
		try
		{
			((ModelManager.CollectObject)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).Destroy();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_CreateGameObject(IntPtr L)
	{
		try
		{
			((ModelManager.CollectObject)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).CreateGameObject();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_UpdateGameObject(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			ModelManager.CollectObject collectObject = (ModelManager.CollectObject)objectTranslator.FastGetCSObj(L, 1);
			int num = Lua.lua_gettop(L);
			if (num == 2 && objectTranslator.Assignable<object>(L, 2))
			{
				object param = objectTranslator.GetObject(L, 2, typeof(object));
				collectObject.UpdateGameObject(param);
				return 0;
			}
			if (num == 1)
			{
				collectObject.UpdateGameObject();
				return 0;
			}
		}
		catch (Exception ex)
		...
		return Lua.luaL_error(L, "invalid arguments to ModelManager.CollectObject.UpdateGameObject!");
	}
	...
	private static int _m_UpdateLod(IntPtr L)
	{
		try
		{
			ModelManager.CollectObject obj = (ModelManager.CollectObject)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
	...
	private static int _m_GetObject(IntPtr L)
	{
		...
		{
			ObjectTranslator objectTranslator = ObjectTranslatorPool.Instance.Find(L);
			GameObject o = ((ModelManager.CollectObject)objectTranslator.FastGetCSObj(L, 1)).GetObject();
			objectTranslator.Push(L, o);
			return 1;
		}
		catch (Exception ex)
