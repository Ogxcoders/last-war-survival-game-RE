...
public class LuaArrAccess64Wrap
{
	public static void __Register(IntPtr L)
	{
		ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		Type typeFromHandle = typeof(LuaArrAccess64);
	...
	private static int __CreateInstance(IntPtr L)
	{
		try
		{
			...
			{
				LuaArrAccess64 o = new LuaArrAccess64();
		...
		return Lua.luaL_error(L, "invalid arguments to LuaArrAccess64 constructor!");
	}
	...
	private static int _m_ToString(IntPtr L)
	{
		try
		{
			string str = ((LuaArrAccess64)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).ToString();
	...
	private static int _m_OnPin(IntPtr L)
	{
		try
		{
			LuaArrAccess64 obj = (LuaArrAccess64)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1);
	...
	private static int _m_OnGC(IntPtr L)
	{
		try
		{
			((LuaArrAccess64)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).OnGC();
			return 0;
		}
		catch (Exception ex)
		{
	...
	private static int _m_IsValid(IntPtr L)
	{
		try
		{
			bool value = ((LuaArrAccess64)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).IsValid();
	...
	private static int _m_GetArrayCapacity(IntPtr L)
	{
		try
		{
			uint arrayCapacity = ((LuaArrAccess64)ObjectTranslatorPool.Instance.Find(L).FastGetCSObj(L, 1)).GetArrayCapacity();
