...
public class MethodWrapsCache
{
	...
	public lua_CSFunction GetConstructorWrap(Type type)
	{
		if (!constructorCache.ContainsKey(type))
		{
			...
			else
			{
				...
				{
					bool flag = false;
					for (int num = 0; num < constructors.Length; num++)
					{
						if (constructors[num].GetParameters().Length == 0)
						{
							flag = true;
							break;
						}
	...
	public MethodWrap _GenMethodWrap(Type type, string methodName, IEnumerable<MemberInfo> methodBases, bool forceCheck = false)
	{
		...
		{
			MethodBase methodBase = methodBasis as MethodBase;
			if (!(methodBase == null) && !methodBase.IsGenericMethodDefinition)
			{
				OverloadMethodWrap overloadMethodWrap = new OverloadMethodWrap(translator, type, methodBase);
