...
internal class ArabicFixerTool
{
	...
	internal static string FixLine(string str)
	{
		...
		{
			if (char.IsPunctuation(array2[num]) && CheckInNumberList(array2, num) && ((num > 0 && IsEnglishCharOrNumOrSymbolOrPunctuationOrSpace(array2[num - 1])) || (num < array2.Length - 1 && IsEnglishCharOrNumOrSymbolOrPunctuationOrSpace(array2[num + 1]))))
			{
				list2.Add(array2[num]);
			}
			else if (array2[num] == '-' && ((num < array2.Length - 1 && array2[num + 1] == '>') || (num > 0 && array2[num - 1] == '<')))
			{
				list2.Add(array2[num]);
			}
			else if (((char.IsPunctuation(array2[num]) && CheckInNumberList(array2, num)) || array2[num] == ' ') && num > 0 && num < array2.Length - 1 && IsEnglishCharOrSymbolOrPunctuationOrSpace(array2[num - 1]) && IsEnglishCharOrSymbolOrPunctuationOrSpace(array2[num + 1]))
			{
				list2.Add(array2[num]);
			}
			else if (char.IsNumber(array2[num]) || char.IsLower(array2[num]) || char.IsUpper(array2[num]) || char.IsSymbol(array2[num]) || (char.IsPunctuation(array2[num]) && CheckInNumberList(array2, num)))
			{
				if (array2[num] == '(')
				{
					list2.Add(')');
	...
	private static bool IsEnglishCharOrNumOrSymbolOrPunctuationOrSpace(char value)
	{
		if (!char.IsLower(value) && !char.IsUpper(value) && !char.IsPunctuation(value) && !char.IsNumber(value) && !char.IsSymbol(value))
		{
			return value == ' ';
		}
		return true;
	...
	private static bool IsEnglishCharOrSymbolOrPunctuationOrSpace(char value)
	{
		if (!char.IsLower(value) && !char.IsUpper(value) && !char.IsPunctuation(value) && !char.IsSymbol(value))
		{
			return value == ' ';
		}
		return true;
	...
	private static bool IsEnglishCharOrNumOrSymbolOrPunctuation(char value)
	{
		if (!char.IsLower(value) && !char.IsUpper(value) && !char.IsPunctuation(value) && !char.IsNumber(value))
		{
	...
	private static bool CheckInNumberList(char[] str, int index)
	{
		if (index >= 0 && index < str.Length)
		{
			if (str[index] == '(' || str[index] == ')' || str[index] == '[' || str[index] == ']')
			{
				if (index > 0 && !IsEnglishCharOrNumOrSymbolOrPunctuationOrSpace(str[index - 1]))
				{
					return str[index - 1] == ' ';
				}
				return true;
			...
			if (str[index] == ':')
			{
				if (index > 0 && char.IsDigit(str[index - 1]) && index < str.Length - 1 && char.IsDigit(str[index + 1]))
				{
					return true;
				}
				if (index > 0 && (char.IsLower(str[index - 1]) || char.IsUpper(str[index - 1])))
				{
					return true;
				}
				return false;
			}
			if (str[index] == '.' && index > 0 && char.IsDigit(str[index - 1]) && index < str.Length - 1 && !IsEnglishCharOrNumOrSymbolOrPunctuation(str[index + 1]))
			{
				return false;
			}
			if ((str[index] == '!' || str[index] == '.' || str[index] == '?') && index == str.Length - 1)
			{
				return false;
			}
		}
