...
public class PaintContext
{
	...
	private void FindTerrainTilesUnlimited(bool texelPadding)
	{
		...
		foreach (KeyValuePair<TerrainUtility.TerrainMap.TileCoord, Terrain> terrainTile in terrainMap.m_terrainTiles)
		{
			...
			int num = key.tileX * (targetTextureWidth - (texelPadding ? 1 : 0));
			int num2 = key.tileZ * (targetTextureHeight - (texelPadding ? 1 : 0));
			RectInt other = new RectInt(num, num2, targetTextureWidth, targetTextureHeight);
			if (pixelRect.Overlaps(other))
			{
				m_TerrainTiles.Add(TerrainTile.Make(value, num, num2, pixelRect, targetTextureWidth, targetTextureHeight));
