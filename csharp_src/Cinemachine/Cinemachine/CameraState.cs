...
public struct CameraState
{
	...
	public static CameraState Lerp(CameraState stateA, CameraState stateB, float t)
	{
		...
		if (((stateA.BlendHint | stateB.BlendHint) & BlendHintValue.NoOrientation) == 0)
		{
			...
			{
				v = result.ReferenceLookAt - result.CorrectedPosition;
			}
			if (v.AlmostZero() || ((stateA.BlendHint | stateB.BlendHint) & BlendHintValue.IgnoreLookAtTarget) != BlendHintValue.Nothing)
			{
			...
			{
				v = v.normalized;
				if ((v - result.ReferenceUp).AlmostZero() || (v + result.ReferenceUp).AlmostZero())
				{
					blended = UnityQuaternionExtensions.SlerpWithReferenceUp(stateA.RawOrientation, stateB.RawOrientation, t, result.ReferenceUp);
				}
				else
				{
					blended = Quaternion.LookRotation(v, result.ReferenceUp);
					...
					blended = blended.ApplyCameraRotation(Vector2.Lerp(a, b, t2), result.ReferenceUp);
				}
			}
		}
