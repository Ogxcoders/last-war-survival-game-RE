...
public class CinemachineCollider : CinemachineExtension
{
	...
	{
		return GetExtraState<VcamExtraState>(vcam).colliderDisplacement > 0f;
	}
	...
	protected override void PostPipelineStageCallback(CinemachineVirtualCameraBase vcam, CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
	{
		...
		if (stage == CinemachineCore.Stage.Body && m_AvoidObstacles)
		{
			...
			Vector3 vector = RespectCameraRadius(state.CorrectedPosition + zero, ref state);
			if (num > 0f && deltaTime >= 0f)
			{
				Vector3 initial2 = vector - extra.m_previousDisplacementCorrection;
				initial2 = Damper.Damp(initial2, num, deltaTime);
				vector = extra.m_previousDisplacementCorrection + initial2;
			}
			zero += vector;
			extra.m_previousDisplacementCorrection = vector;
	...
	private Vector3 PreserveLignOfSight(ref CameraState state, ref VcamExtraState extra)
	{
		...
		if (state.HasLookAt && (int)m_CollideAgainst != 0 && (int)m_CollideAgainst != (int)m_TransparentLayers)
		{
			...
			vector = PullCameraInFrontOfNearestObstacle(correctedPosition, referenceLookAt, (int)m_CollideAgainst & ~(int)m_TransparentLayers, ref hitInfo);
			...
			if (hitInfo.collider != null)
			{
				...
				if (m_Strategy != ResolutionStrategy.PullCameraForward)
				{
					...
					vector2 = PushCameraBack(vector2, pushDir, hitInfo, referenceLookAt, new Plane(state.ReferenceUp, correctedPosition), pushDir.magnitude, m_MaximumEffort, ref extra);
				}
			}
			vector = vector2 - correctedPosition;
			if (m_SmoothingTime > 0.0001f)
			{
				Vector3 vector3 = vector2 - referenceLookAt;
	...
	private Vector3 PullCameraInFrontOfNearestObstacle(Vector3 cameraPos, Vector3 lookAtPos, int layerMask, ref RaycastHit hitInfo)
	{
		...
		if (magnitude > 0.0001f)
		{
			...
			else
			{
				...
				num2 += 0.001f;
				if (num2 > 0.0001f && RaycastIgnoreTag(ray, out hitInfo, num2, layerMask))
				{
	...
	private bool RaycastIgnoreTag(Ray ray, out RaycastHit hitInfo, float rayLength, int layerMask)
	{
		...
		while (Physics.Raycast(ray, out hitInfo, rayLength, layerMask, QueryTriggerInteraction.Ignore))
		{
			...
			num += rayLength - (hitInfo.distance - 0.001f);
	...
	private Vector3 PushCameraBack(Vector3 currentPos, Vector3 pushDir, RaycastHit obstacle, Vector3 lookAtPos, Plane startPlane, float targetDistance, int iterations, ref VcamExtraState extra)
	{
		...
		if (RaycastIgnoreTag(ray, out var hitInfo, pushBackDistance, (int)m_CollideAgainst & ~(int)m_TransparentLayers))
		{
		...
		if (magnitude < 0.0001f || RaycastIgnoreTag(new Ray(lookAtPos, outDir), out var _, magnitude - 0.001f, (int)m_CollideAgainst & ~(int)m_TransparentLayers))
		{
			return currentPos;
		}
		...
		if (pushBackDistance > 0.0001f)
		{
			if (!RaycastIgnoreTag(ray, out hitInfo, pushBackDistance, (int)m_CollideAgainst & ~(int)m_TransparentLayers))
			{
				vector = ray.GetPoint(pushBackDistance);
				extra.AddPointToDebugPath(vector);
			}
	...
	private Vector3 RespectCameraRadius(Vector3 cameraPos, ref CameraState state)
	{
		...
		if (num == 0 && (int)m_TransparentLayers != 0 && magnitude > m_MinimumDistanceFromTarget + 0.0001f)
		{
			...
			Vector3 origin = state.ReferenceLookAt + vector2 * m_MinimumDistanceFromTarget;
			if (RaycastIgnoreTag(new Ray(origin, vector2), out hitInfo, num2, m_CollideAgainst))
			{
		...
		if ((num > 0 && magnitude == 0f) || magnitude > m_MinimumDistanceFromTarget)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (m_IgnoreTag.Length <= 0 || !collider2.CompareTag(m_IgnoreTag))
				{
					...
					if (magnitude > m_MinimumDistanceFromTarget && collider2.Raycast(ray, out hitInfo, magnitude + m_CameraRadius))
					{
						vector3 = ray.GetPoint(hitInfo.distance) - cameraPos - vector2 * 0.001f;
					}
					if (Physics.ComputePenetration(mCameraCollider, cameraPos + vector3, Quaternion.identity, collider2, collider2.transform.position, collider2.transform.rotation, out var direction, out var distance))
					{
						vector += direction * distance + vector3;
					}
				}
			}
		}
	...
	private bool CheckForTargetObstructions(CameraState state)
	{
		if (state.HasLookAt)
		{
			...
			if (RaycastIgnoreTag(ray, out var _, magnitude - m_MinimumDistanceFromTarget, (int)m_CollideAgainst & ~(int)m_TransparentLayers))
			{
				return true;
			}
		}
