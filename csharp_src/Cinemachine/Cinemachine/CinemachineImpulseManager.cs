	...
	public class ImpulseEvent
	{
		...
		public bool Expired
		{
			get
			{
				...
				if (duration > 0f)
				{
					return m_StartTime + duration <= Time.time;
				}
				return false;
			}
		}
		...
		public bool GetDecayedSignal(Vector3 listenerPosition, bool use2D, out Vector3 pos, out Quaternion rot)
		{
			if (m_SignalSource != null)
			{
				...
				float num2 = (use2D ? Vector2.Distance(listenerPosition, m_Position) : Vector3.Distance(listenerPosition, m_Position));
				float num3 = m_Envelope.GetValueAt(num) * DistanceDecay(num2);
				m_SignalSource.GetSignal(num, out pos, out rot);
				pos *= num3;
				rot = Quaternion.SlerpUnclamped(Quaternion.identity, rot, num3);
				if (m_DirectionMode == DirectionMode.RotateTowardSource && num2 > 0.0001f)
				{
					Quaternion quaternion = Quaternion.FromToRotation(Vector3.up, listenerPosition - m_Position);
					if (m_Radius > 0.0001f)
					{
						float num4 = Mathf.Clamp01(num2 / m_Radius);
						quaternion = Quaternion.Slerp(quaternion, Quaternion.identity, Mathf.Cos(MathF.PI * num4 / 2f));
					}
					pos = quaternion * pos;
					rot = Quaternion.Inverse(quaternion) * rot * quaternion;
				}
				return true;
			}
			pos = Vector3.zero;
