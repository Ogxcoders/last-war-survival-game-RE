...
public class DefaultContractResolver : IContractResolver
{
	...
	private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
	{
		...
		{
			Type type4 = typeof(EnumerableDictionaryWrapper<, >).MakeGenericType(type, type2);
			ConstructorInfo method = type4.GetConstructors().FirstOrDefault() ?? throw new JsonException("Missing constructor for enumerator type " + type4.FullName + ". Perhaps it got stripped?");
