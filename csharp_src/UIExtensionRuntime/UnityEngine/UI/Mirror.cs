...
public class Mirror : BaseMeshEffect
{
	...
	{
		if (!(base.graphic != null) || !(base.graphic is Image))
		{
			return;
		}
		...
		if (overrideSprite != null)
		{
			...
			float num2 = overrideSprite.rect.height / (base.graphic as Image).pixelsPerUnit;
			rectTransform.anchorMax = rectTransform.anchorMin;
			switch (m_MirrorType)
			{
			case MirrorType.Horizontal:
				rectTransform.sizeDelta = new Vector2(num * 2f, num2);
				break;
			case MirrorType.Vertical:
				rectTransform.sizeDelta = new Vector2(num, num2 * 2f);
				break;
			case MirrorType.Quarter:
				rectTransform.sizeDelta = new Vector2(num * 2f, num2 * 2f);
				break;
			}
			base.graphic.SetVerticesDirty();
		}
	...
	protected void DrawSimple(List<UIVertex> output, int count)
	{
		...
		SimpleScale(pixelAdjustedRect, output, count);
		switch (m_MirrorType)
		{
		case MirrorType.Horizontal:
		...
		case MirrorType.Vertical:
			ExtendCapacity(output, count);
			MirrorVerts(pixelAdjustedRect, output, count, isHorizontal: false);
			break;
		case MirrorType.Quarter:
			ExtendCapacity(output, count * 3);
			MirrorVerts(pixelAdjustedRect, output, count);
			MirrorVerts(pixelAdjustedRect, output, count * 2, isHorizontal: false);
			break;
		}
	}
	...
	protected void DrawSliced(List<UIVertex> output, int count)
	{
		...
		Rect pixelAdjustedRect = base.graphic.GetPixelAdjustedRect();
		SlicedScale(pixelAdjustedRect, output, count);
	...
	protected void DrawTiled(List<UIVertex> verts, int count)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			float center2 = GetCenter(value.position.y, value2.position.y, value3.position.y);
			if ((m_MirrorType == MirrorType.Horizontal || m_MirrorType == MirrorType.Quarter) && Mathf.FloorToInt((center - pixelAdjustedRect.xMin) / num) % 2 == 1)
			{
			...
			if ((m_MirrorType == MirrorType.Vertical || m_MirrorType == MirrorType.Quarter) && Mathf.FloorToInt((center2 - pixelAdjustedRect.yMin) / num2) % 2 == 1)
			{
				value.uv0 = GetOverturnUV(value.uv0, innerUV.y, innerUV.w, isHorizontal: false);
				value2.uv0 = GetOverturnUV(value2.uv0, innerUV.y, innerUV.w, isHorizontal: false);
				value3.uv0 = GetOverturnUV(value3.uv0, innerUV.y, innerUV.w, isHorizontal: false);
			}
			verts[i * 3] = value;
			verts[i * 3 + 1] = value2;
			verts[i * 3 + 2] = value3;
	...
	protected void SlicedScale(Rect rect, List<UIVertex> verts, int count)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (m_MirrorType == MirrorType.Vertical || m_MirrorType == MirrorType.Quarter)
			{
				if (num2 < adjustedBorders.y && position.y >= rect.center.y)
				{
					position.y = rect.center.y;
				}
				else if (position.y >= adjustedBorders.y)
				{
					position.y = (position.y + rect.y) * 0.5f;
				}
			}
