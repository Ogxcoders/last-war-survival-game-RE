...
public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEventSystemHandler, IEndDragHandler, IDragHandler, IPointerClickHandler
{
	...
	private Action<LoopListViewItem2> mOnRecycleItemFunc;

	private Action<LoopListViewItem2, bool> mArabicMirrorCallback;
	...
	private bool mUseCanvas;

	private LoopListViewItemProvider mItemProvider;

	private bool mStopAdjustVelocity;

	private bool mSmoothDraggingInertia;

	private bool mAutoLoad;

	private int mAutoLoadRemain = 10;

	private Action mAutoLoadPrev;

	private Action mAutoLoadLast;
	...
	public Func<LoopListView2, int, string> OnGetItemNameByIndex
	{
		set
		{
			mOnGetItemNameByIndex = value;
		}
	}

	public Action<LoopListViewItem2> OnRecycleItemFunc
	{
		set
		{
			mOnRecycleItemFunc = value;
		}
	}
	...
	public void SetItemUseCanvas(bool use)
	{
		mUseCanvas = use;
	}

	public void SetStopAdjustVelocity(bool stop)
	{
		mStopAdjustVelocity = stop;
	}

	public void SetSmoothDraggingInertia(bool value)
	{
		mSmoothDraggingInertia = value;
		if (mScrollRect != null)
		{
			mScrollRect.InertiaSkipNormalizePosition = value;
		}
	}

	public void SetAutoLoad(int remain, Action loadPrev, Action loadLast)
	{
		mAutoLoad = remain > 0;
		mAutoLoadRemain = remain;
		mAutoLoadPrev = loadPrev;
		mAutoLoadLast = loadLast;
	}
	...
	public void SetOnSnapItemFinished(Action<LoopListView2, LoopListViewItem2> callback)
	{
		mOnSnapItemFinished = callback;
	}

	public void SetOnSnapNearestChanged(Action<LoopListView2, LoopListViewItem2> callback)
	{
	...
	public void OnItemPrefabChanged(string prefabName)
	{
		...
		if (mItemPoolDict.TryGetValue(prefabName, out value))
		{
			...
			value.Init(itemPrefabConfData.mItemPrefab.name, itemPrefabConfData, mContainerTrans, DoArabicMirror);
			if (num >= 0)
			{
	...
	public void InitListView(int itemTotalCount, Func<LoopListView2, int, LoopListViewItem2> onGetItemByIndex, LoopListViewInitParam initParam = null, Func<LoopListView2, int, string> onGetItemNameByIndex = null, Action<LoopListViewItem2, bool> arabicMirrorCallback = null, Action<LoopListViewItem2> onRecycleItemFunc = null)
	{
		if (mListViewInited)
		{
			mOnGetItemNameByIndex = onGetItemNameByIndex;
			mOnGetItemByIndex = onGetItemByIndex;
			mArabicMirrorCallback = arabicMirrorCallback;
			mOnRecycleItemFunc = onRecycleItemFunc;
			return;
		}
		if (initParam != null)
		{
		...
		mItemProvider = base.gameObject.GetComponent<LoopListViewItemProvider>();
		if (mItemProvider != null)
		{
			mItemProvider.SetHost(this);
		}
	...
	public void SetListItemCount(int itemCount, bool resetPos = true, bool needMoveToIndex = true)
	{
		...
		else if (needMoveToIndex)
		{
			MovePanelToItemIndex(num, 0f);
		}
	}

	public void ClearItemPosCache()
	{
		mItemPosMgr.SetItemMaxCount(0);
	}

	public void InsertFront_Mod(int itemCount)
	{
		int num = itemCount - mItemTotalCount;
		mCurSnapData.Clear();
		mItemTotalCount = itemCount;
		ItemPosMgr itemPosMgr = new ItemPosMgr(mItemDefaultWithPaddingSize);
		itemPosMgr.SetItemMaxCount(mItemTotalCount);
		if (mIsGoTail && mFocusIndex >= 0)
		{
			mFocusIndex += num;
		}
		for (int i = 0; i < mItemPosMgr.mMaxCount; i++)
		{
			float itemSize = mItemPosMgr.GetItemSize(i);
			itemPosMgr.SetItemSize(i + num, itemSize);
		}
		itemPosMgr.Update(updateAll: true);
		for (int j = 0; j < mItemList.Count; j++)
		{
			mItemList[j].ItemIndex += num;
		}
		mCurReadyMaxItemIndex = mItemTotalCount;
		mNeedCheckNextMaxItem = false;
		mNeedCheckNextMinItem = false;
		mItemPosMgr = itemPosMgr;
		mLeftSnapUpdateExtraCount = 1;
		if (mItemList.Count > 0)
		{
			int num2 = mItemTotalCount - 1;
			if (mItemList[mItemList.Count - 1].ItemIndex <= num2)
			{
				UpdateContentSize();
				UpdateAllShownItemsPos();
			}
	...
	public void AppendTail_Mod(int itemCount)
	{
		mCurSnapData.Clear();
		mItemTotalCount = itemCount;
		ItemPosMgr itemPosMgr = new ItemPosMgr(mItemDefaultWithPaddingSize);
		...
		{
			float itemSize = mItemPosMgr.GetItemSize(i);
			itemPosMgr.SetItemSize(i, itemSize);
		}
		itemPosMgr.Update(updateAll: true);
		mCurReadyMaxItemIndex = mItemTotalCount;
		mNeedCheckNextMaxItem = false;
	...
	public void SetListItemCount_Mod(int itemCount, bool resetPos = true, bool needMoveToIndex = true, bool refresh = false)
	{
		_ = mItemTotalCount;
		mCurSnapData.Clear();
		mItemTotalCount = itemCount;
		...
		else if (mItemList[mItemList.Count - 1].ItemIndex <= num)
		{
			UpdateContentSize();
			UpdateAllShownItemsPos();
		}
	...
	private void DoArabicMirror(LoopListViewItem2 item)
	{
		mArabicMirrorCallback?.Invoke(item, mArrangeType == ListItemArrangeType.TopToBottom || mArrangeType == ListItemArrangeType.BottomToTop);
	}
	...
	public LoopListViewItem2 NewListViewItem(string itemPrefabName)
	{
		...
		if (!mItemPoolDict.TryGetValue(itemPrefabName, out value))
		{
			...
			if (!mItemProvider.AddItemPool(itemPrefabName))
			{
				return null;
			}
			if (!mItemPoolDict.TryGetValue(itemPrefabName, out value))
		...
		bool num = value.CacheCount > 0;
		LoopListViewItem2 item = value.GetItem();
		if (!num && value.IsPrefab)
		{
			DoArabicMirror(item);
		}
	...
	{
		Vector3 result = mContainerTrans.localPosition;
		if (mOnGetItemNameByIndex == null)
		{
			return result;
		}
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			...
			{
				return new Vector3(result.x, 0f, 0f);
			}
			...
			num = 0f - offset;
			ItemPrefabConfData itemPrefabConfData = null;
			for (int i = 0; i < itemIndex; i++)
			{
				empty = mOnGetItemNameByIndex(this, itemIndex);
				...
				{
					if (mItemPrefabDataList[j] != null && mItemPrefabDataList[j].mItemPrefab.name.Equals(empty))
					{
						itemPrefabConfData = mItemPrefabDataList[j];
						break;
					}
				}
				if (itemPrefabConfData.mItemHeight <= 0f)
				{
				...
				{
					num += itemPrefabConfData.mItemHeight;
				}
				num += itemPrefabConfData.mPadding;
			}
			result = new Vector3(result.x, num, result.z);
		}
		return result;
	}

	public void MovePanelToItemIndex(int itemIndex, float offset, bool reverse = false)
	{
		...
		ListItemArrangeType listItemArrangeType = mArrangeType;
		if (reverse)
		{
			switch (listItemArrangeType)
			{
			case ListItemArrangeType.TopToBottom:
				listItemArrangeType = ListItemArrangeType.BottomToTop;
				break;
			case ListItemArrangeType.BottomToTop:
				listItemArrangeType = ListItemArrangeType.TopToBottom;
				break;
			case ListItemArrangeType.LeftToRight:
				listItemArrangeType = ListItemArrangeType.LeftToRight;
				break;
		...
		case ListItemArrangeType.TopToBottom:
		{
			float num3 = mContainerTrans.localPosition.y;
			if (num3 < 0f)
			{
				num3 = 0f;
			}
			zero.y = 0f - num3 - offset;
			break;
		}
		case ListItemArrangeType.BottomToTop:
		{
			float num2 = mContainerTrans.localPosition.y;
			if (num2 > 0f)
			{
				num2 = 0f;
			}
		...
		case ListItemArrangeType.LeftToRight:
		{
			float num4 = mContainerTrans.localPosition.x;
			if (num4 > 0f)
			{
				num4 = 0f;
			}
		...
		case ListItemArrangeType.RightToLeft:
		{
			float num = mContainerTrans.localPosition.x;
			if (num < 0f)
			{
				num = 0f;
			}
			zero.x = 0f - num - offset;
			break;
		}
		}
		RecycleAllItem();
	...
	public void MovePanelToItemIndex_Mod(int itemIndex, float offset, bool reverse = false)
	{
		mScrollRect.StopMovement();
		mCurSnapData.Clear();
		if (itemIndex < 0 || mItemTotalCount == 0)
		...
		mItemList.Add(newItemByIndex);
		UpdateContentSize();
		float distanceForNew = viewPortSize;
		float distanceForNew2 = viewPortSize;
		if (mIsVertList)
		{
			newItemByIndex.CachedRectTransform.GetWorldCorners(mItemWorldCorners);
			...
			distanceForNew = vector.y + 100f;
			distanceForNew2 = vector2.y + 100f;
		}
		UpdateListView(viewPortSize + 100f, viewPortSize + 100f, distanceForNew, distanceForNew2);
		AdjustPanelPos();
		ClearAllTmpRecycledItem();
	}

	public void MovePanelToItemIndexToBottom(int itemIndex)
	{
		mScrollRect.StopMovement();
		mCurSnapData.Clear();
		if (itemIndex < 0 || mItemTotalCount == 0)
		...
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			float num = mContainerTrans.localPosition.y - (mContainerTrans.rect.height - mViewPortRectTransform.rect.height);
			float num2 = mContainerTrans.localPosition.y + mViewPortRectTransform.rect.height - num;
			if (mItemList[mItemList.Count - 1].ItemIndex == itemIndex)
			{
				LoopListViewItem2 shownItemByItemIndex = GetShownItemByItemIndex(itemIndex);
				num2 = mContainerTrans.rect.height - shownItemByItemIndex.ItemSizeWithPadding;
			}
			if (num2 < 0f)
			{
				num2 = 0f;
	...
	private void RecycleItemTmp(LoopListViewItem2 item)
	{
		...
		{
			if (mOnRecycleItemFunc != null)
			{
				mOnRecycleItemFunc(item);
			}
	...
	private void InitItemPool()
	{
		foreach (ItemPrefabConfData mItemPrefabData in mItemPrefabDataList)
		{
			...
			InitItemPool(prefabName, mItemPrefabData);
		}
	}

	private void InitItemPool(string prefabName, ItemPrefabConfData data)
	{
		if (mItemPoolDict.ContainsKey(prefabName))
		{
			return;
		}
		RectTransform component = data.mItemPrefab.GetComponent<RectTransform>();
		if (component == null)
		{
		...
		itemPool.mUseCanvas = mUseCanvas;
		itemPool.Init(prefabName, data, mContainerTrans, DoArabicMirror);
		mItemPoolDict.Add(prefabName, itemPool);
		mItemPoolList.Add(itemPool);
	}

	public void AddItemPoolByItemProvider(string prefabName, ItemPrefabConfData data)
	{
		...
		{
			InitItemPool(prefabName, data);
		}
	}

	public void EnableLoadingGoTail(bool enable, int focusIndex = -1, float offset = 0f)
	{
		if (!mIsDraging)
		{
			mIsGoTail = enable;
	...
	private void LoadingGoTail()
	{
		if (mIsGoTail)
		{
			float height = mContainerTrans.rect.height;
			float height2 = mViewPortRectTransform.rect.height;
			SetVelocity(new Vector2(0f, 0f));
			float num = 0f;
			if (mFocusIndex == -1 || mItemTotalCount <= mFocusIndex)
			{
				num = ((height2 > height) ? 0f : (height - height2));
			}
			else
			{
				float itemPos = GetItemPos(mFocusIndex);
				num = ((height2 > height) ? 0f : Math.Min(height - height2, itemPos));
			}
			SetContainerLocalPosY(num + mOffset);
		}
	}
	...
	private void AdjustPanelPos()
	{
		...
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			...
			if (num > 0f)
			{
				Vector3 localPosition = mItemList[0].CachedRectTransform.localPosition;
				localPosition.y -= num;
				mItemList[0].CachedRectTransform.localPosition = localPosition;
				UpdateAllShownItemsPos();
			}
		}
		else if (mArrangeType == ListItemArrangeType.BottomToTop)
		{
			...
			if (num2 > 0f)
			{
				...
				mItemList[0].CachedRectTransform.localPosition = localPosition2;
				UpdateAllShownItemsPos();
			}
		}
		else if (mArrangeType == ListItemArrangeType.LeftToRight)
		...
		else
		{
			...
			if (num4 > 0f)
			{
				...
				localPosition4.x -= num4;
				mItemList[0].CachedRectTransform.localPosition = localPosition4;
				UpdateAllShownItemsPos();
			}
		}
	}
	...
	public void UpdateAllShownItemSnapData()
	{
		...
		_ = mContainerTrans.localPosition;
	...
	private void UpdateSnapVertical(bool immediate = false)
	{
		...
		if (flag)
		{
			...
			if (num >= 0)
			{
				...
				if (mItemList[num].ItemIndex != num8 && mOnSnapNearestChanged != null)
				{
		...
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			...
			SetContainerLocalPosY(localPosition.y);
		}
		else if (mArrangeType == ListItemArrangeType.BottomToTop)
		{
	...
	private void UpdateCurSnapData()
	{
		...
		{
			LoopListViewItem2 shownItemByItemIndex = GetShownItemByItemIndex(mCurSnapData.mSnapTargetIndex);
			if (shownItemByItemIndex == null)
			{
				mCurSnapData.Clear();
				return;
			...
			mCurSnapData.mTargetSnapVal = shownItemByItemIndex.DistanceWithViewPortSnapCenter;
	...
	public void UpdateListView(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
	{
		...
		while (flag3)
		{
			...
			flag3 = UpdateForHorizontalList(distanceForRecycle0, distanceForRecycle1, distanceForNew0, distanceForNew1);
			flag4 = flag4 || flag3;
		}
		if (flag4)
		{
	...
	private bool UpdateForVertList(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
	{
		...
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			if (mItemList.Count == 0)
			{
				float num = mContainerTrans.localPosition.y;
				if (num < 0f)
				{
					num = 0f;
				}
			...
			if (mViewPortRectLocalCorners[0].y - vector4.y < distanceForNew1)
			{
				...
				if (num2 <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
				{
					...
					if (!(newItemByIndex2 == null))
					{
						...
						float y = loopListViewItem2.CachedRectTransform.localPosition.y - loopListViewItem2.CachedRectTransform.rect.height - loopListViewItem2.Padding;
			...
			if (vector.y - mViewPortRectLocalCorners[1].y < distanceForNew0)
			{
				...
				if (num3 >= mCurReadyMinItemIndex || mNeedCheckNextMinItem)
				{
					...
					if (!(newItemByIndex3 == null))
					{
						...
						float y2 = loopListViewItem.CachedRectTransform.localPosition.y + newItemByIndex3.CachedRectTransform.rect.height + newItemByIndex3.Padding;
		...
		else
		{
			if (mItemList.Count == 0)
			{
				float num4 = mContainerTrans.localPosition.y;
				if (num4 > 0f)
				{
					num4 = 0f;
				}
			...
			if (vector7.y - mViewPortRectLocalCorners[1].y < distanceForNew1)
			{
				...
				if (num5 <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
				{
					...
					if (!(newItemByIndex5 == null))
					{
						...
						float y3 = loopListViewItem4.CachedRectTransform.localPosition.y + loopListViewItem4.CachedRectTransform.rect.height + loopListViewItem4.Padding;
			...
			if (mViewPortRectLocalCorners[0].y - vector6.y < distanceForNew0)
			{
				...
				if (num6 >= mCurReadyMinItemIndex || mNeedCheckNextMinItem)
				{
					...
					float y4 = loopListViewItem3.CachedRectTransform.localPosition.y - newItemByIndex6.CachedRectTransform.rect.height - newItemByIndex6.Padding;
			...
			if (vector4.x - mViewPortRectLocalCorners[2].x < distanceForNew1)
			{
				...
				if (num2 <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
				{
					...
					if (!(newItemByIndex2 == null))
					{
						...
						mItemList.Add(newItemByIndex2);
						float x = loopListViewItem2.CachedRectTransform.localPosition.x + loopListViewItem2.CachedRectTransform.rect.width + loopListViewItem2.Padding;
			...
			if (mViewPortRectLocalCorners[1].x - vector.x < distanceForNew0)
			{
				...
				if (num3 >= mCurReadyMinItemIndex || mNeedCheckNextMinItem)
				{
					...
					if (!(newItemByIndex3 == null))
					{
						...
						mItemList.Insert(0, newItemByIndex3);
						float x2 = loopListViewItem.CachedRectTransform.localPosition.x - newItemByIndex3.CachedRectTransform.rect.width - newItemByIndex3.Padding;
			...
			if (mViewPortRectLocalCorners[1].x - vector7.x < distanceForNew1)
			{
				...
				if (num5 <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
				{
					...
					if (!(newItemByIndex5 == null))
					{
						...
						mItemList.Add(newItemByIndex5);
						float x3 = loopListViewItem4.CachedRectTransform.localPosition.x - loopListViewItem4.CachedRectTransform.rect.width - loopListViewItem4.Padding;
			...
			if (vector6.x - mViewPortRectLocalCorners[2].x < distanceForNew0)
			{
				...
				if (num6 >= mCurReadyMinItemIndex || mNeedCheckNextMinItem)
				{
					...
					if (!(newItemByIndex6 == null))
					{
						...
						mItemList.Insert(0, newItemByIndex6);
						float x4 = loopListViewItem3.CachedRectTransform.localPosition.x + newItemByIndex6.CachedRectTransform.rect.width + newItemByIndex6.Padding;
	...
	private void UpdateAllShownItemsPos()
	{
		...
		if (mArrangeType == ListItemArrangeType.TopToBottom)
		{
			...
			float y = mItemList[0].CachedRectTransform.localPosition.y;
			float num2 = num - y;
			float num3 = num;
			for (int i = 0; i < count; i++)
			{
			...
			if (num2 != 0f)
			{
				Vector2 vector = mContainerTrans.localPosition;
				vector.y -= num2;
				SetContainerLocalPosY(vector.y);
			}
		}
		else if (mArrangeType == ListItemArrangeType.BottomToTop)
		{
			...
			float y2 = mItemList[0].CachedRectTransform.localPosition.y;
			float num5 = num4 - y2;
			float num6 = num4;
			for (int j = 0; j < count; j++)
			{
		...
		else if (mArrangeType == ListItemArrangeType.LeftToRight)
		{
			...
			float x = mItemList[0].CachedRectTransform.localPosition.x;
			float num8 = num7 - x;
			float num9 = num7;
			for (int k = 0; k < count; k++)
			{
		...
		else if (mArrangeType == ListItemArrangeType.RightToLeft)
		{
			...
			float x2 = mItemList[0].CachedRectTransform.localPosition.x;
		...
		if (mIsDraging)
		{
			...
			if (mSmoothDraggingInertia)
			{
				Vector2 prevPosition = mScrollRect.GetPrevPosition();
				mScrollRect.Rebuild(CanvasUpdate.PostLayout);
				mScrollRect.SetPrevPosition(prevPosition);
			}
			else
			{
				mScrollRect.Rebuild(CanvasUpdate.PostLayout);
			}
			if (!mStopAdjustVelocity)
			{
				SetVelocity(mAdjustedVec);
				mNeedAdjustVec = true;
			}
	...
	private void CanCleItemClickAnimation()
	{
		...
		if (!(null != currentSelectedGameObject) || mItemList == null || mItemList.Count <= 0)
		{
			return;
		}
		for (int i = 0; i < mItemList.Count; i++)
		{
			if (null != mItemList[i] && currentSelectedGameObject.transform.IsChildOf(mItemList[i].transform))
			{
	...
	public void ClearAllItems()
	{
		...
		for (int i = 0; i < mItemList.Count; i++)
		{
			if (!hashSet.Contains(mItemList[i].ItemPrefabName))
			{
				hashSet.Add(mItemList[i].ItemPrefabName);
			}
		}
		SetListItemCount(0);
		...
		{
			ItemPool value = null;
			if (!mItemPoolDict.TryGetValue(item, out value))
			{
				break;
			}
			value.DestroyAllItem();
	...
	private void SetContainerLocalPosY(float y)
	{
		...
		if (y > height + 10f)
		{
			Debug.LogError($"#LoopList# SetContainerLocalPosY异常!!! srcPos.y:{vector.y}, newY:{y}, height:{height}");
		}
		vector.y = y;
		if (mSmoothDraggingInertia)
		{
			mScrollRect.OffsetPrevPosition(LocalToAnchorPosition(mContainerTrans, vector));
		}
		mContainerTrans.localPosition = vector;
	}

	private Vector2 LocalToAnchorPosition(RectTransform rect, Vector2 localPosition2D)
	{
		...
		Vector2 vector = rectTransform.rect.min + Vector2.Scale(rect.anchorMin, rectTransform.rect.size);
		Vector2 vector2 = rect.rect.min + localPosition2D - vector;
		Vector2 vector3 = rectTransform.rect.max - Vector2.Scale(Vector2.one - rect.anchorMax, rectTransform.rect.size);
		Vector2 a = rect.rect.max + localPosition2D - vector3 - vector2;
		return vector2 + Vector2.Scale(a, rect.pivot);
	}

	private void SetVelocity(Vector2 velocity)
	{
		Vector2 velocity2 = mScrollRect.velocity;
		if (Math.Abs(velocity.y - velocity2.y) > 5000f)
		{
			Debug.LogError($"#LoopList# SetVelocity异常!!!  srcVelocity.y:{velocity2.y}, newVelocity:{velocity.y}");
		}
	...
	public void InitArabicItemPrefabData()
	{
		foreach (ItemPrefabConfData mItemPrefabData in mItemPrefabDataList)
		{
			if (mArrangeType == ListItemArrangeType.TopToBottom || mArrangeType == ListItemArrangeType.BottomToTop)
			{
				mItemPrefabData.mStartPosOffset = 0f - mItemPrefabData.mStartPosOffset;
			}
		}
	}
}
