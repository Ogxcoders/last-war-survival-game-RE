...
public class ZipFile : IEnumerable, IDisposable
{
	...
	private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
	{
		lock (baseStream_)
		{
			...
			if (num)
			{
				...
				if (entry.CompressionMethod != (CompressionMethod)num7)
				{
				...
				if ((num6 & 0x40) != 0 && num5 < 62)
				{
					throw new ZipException("Strong encryption flag set but version not high enough");
				}
				if ((num6 & 0x2000) != 0 && (num8 != 0 || num9 != 0))
				{
	...
	{
		int result = -1;
		string text = (entry.IsDirectory ? GetTransformedDirectoryName(entry.Name) : GetTransformedFileName(entry.Name));
	...
	private int FindExistingUpdate(string fileName)
	{
		...
		{
			result = updateIndex_[transformedFileName];
		}
		return result;
	}
	...
	private void CopyEntryDirect(ZipFile workFile, ZipUpdate update, ref long destinationPosition)
	{
		...
		if (num)
		{
			...
			{
				destinationPosition += num2 - num3 + 26 + update.Entry.CompressedSize + GetDescriptorSize(update);
			}
			return;
		}
	...
	private void ReadEntries()
	{
		...
		for (ulong num10 = 0uL; num10 < num4; num10++)
		{
			...
			{
				zipEntry.CryptoCheckValue = (byte)((num12 >> 8) & 0xFF);
			}
			if (num17 > 0)
			{
