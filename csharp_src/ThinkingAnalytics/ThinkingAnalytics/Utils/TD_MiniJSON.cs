...
public class TD_MiniJSON
{
	private sealed class Parser : IDisposable
	{
		...
		private object ParseByToken(TOKEN token)
		{
			...
			{
				string text = ParseString();
				if (DateTime.TryParseExact(text, "yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result))
				{
					return result;
				}
				return text;
		...
		private object ParseNumber()
		{
			...
			if (!double.TryParse(nextWord, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out var result2))
			{
				double.TryParse(nextWord, NumberStyles.AllowDecimalPoint, CultureInfo.CreateSpecificCulture("es-ES"), out result2);
			}
			return result2;
		}
	...
	private sealed class Serializer
	{
		...
		private Func<DateTime, string> func;
